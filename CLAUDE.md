...
Crackerjack's MCP server adopts the `apply_tool_profile()` dispatcher from
[mcp-common](https://github.com/lesleslie/mcp-common) (>=0.18.0). Operators
select a profile tier via the `CRACKERJACK_TOOL_PROFILE` env var (defaults to
`FULL` to preserve the pre-W2a behavior).

| Tier | What registers | Operator profile |
|------|----------------|------------------|
| **MINIMAL** | `health_tools` only | K8s liveness probe, read-only monitoring |
| **STANDARD** | MINIMAL + every register fn used by daily `python -m crackerjack run` (coverage, ruff, pytest, security, formatting, complexity) | Day-to-day dev |
| **FULL** | All tool groups (adds the AI-assisted auto-fix / hooks / jobs surface) | Full power-user |

The discover_tools meta-tool is registered unconditionally so clients can
