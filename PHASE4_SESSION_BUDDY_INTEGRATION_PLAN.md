...
class SessionMetrics:
    ...
    conventional_commit_compliance: float | None = (
        None  # % following conventional commits
    )
...
class GitMetricsSessionCollector:
    ...
    async def collect_session_metrics(self) -> SessionMetrics:
        ...
        commits_last_hour = await self.collector.get_commit_velocity(
            repo_path=str(self.project_path), hours_back=1
        )
        ...
        merge_stats = await self.collector.get_merge_statistics(
            repo_path=str(self.project_path), days_back=30
        )
...
1. Bottleneck identification (slow code review, long-lived branches)
1. Quality correlation (velocity vs. test pass rates)
1. Actionable recommendations with expected impact
...
1. **Create GitMetricsSessionCollector** ✅
1. **Create WorkflowOptimizationEngine** ✅
1. **Extend Session-Buddy MCP Client** ✅
...
1. **Bottleneck Detection**: Automatically identify slow code reviews, long-lived branches
1. **Quality Correlation**: Link git practices (conventional commits, branch hygiene) with test outcomes
1. **Team Analytics**: Compare velocity across sessions and repositories
