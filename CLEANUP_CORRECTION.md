...
1. **Missed dynamic imports**: Some modules imported via `from .submodule import`
1. **Didn't check handler files**: CLI handlers in `cli/handlers/` use these modules
1. **Assumed without verifying**: Should have traced actual execution paths
...
1. **Didn't trace execution**: Should have followed CLI option → handler → module
1. **Assumed 0% coverage = unused**: Not true for supporting infrastructure
1. **Missed handler files**: `cli/handlers/*.py` contain the real usage

### Correct Methodology

1. **Start from CLI options**: Identify feature → trace to implementation
1. **Check handler files**: `cli/handlers/*.py` show real usage
1. **Trace import chains**: Use proper module resolution
1. **Verify execution paths**: Not just imports, but actual function calls
