...
def find_server_processes() -> list[psutil.Process]:
    ...
    for proc in psutil.process_iter(["pid", "name", "cmdline"]):
        try:
            ...
            is_session_mcp = (
                "session_mgmt_mcp" in cmdline_str or
                "session-mgmt-mcp" in cmdline_str
            )
            
            # Check if it's running a server (various ways to start)
            is_server = (
                "--start-server" in cmdline_str or
                "server.py" in cmdline_str or
                # Also check if it's bound to our ports
                any(conn.laddr.port in [8678, 8677] for conn in proc.net_connections() if conn.laddr)
            )
...
def show_status() -> None:
    ...
    if server_status["running"]:
        table.add_row("HTTP Endpoint", f"http://127.0.0.1:{server_status['http_port']}/mcp")
        table.add_row("WebSocket Monitor", f"ws://127.0.0.1:{server_status['websocket_port']}")
...
def start_mcp_server(
    ...
    try:
        ...
        cmd = [sys.executable, "-c", 
               "from session_mgmt_mcp.server import main; main()"]
        ...
        if process.poll() is None:  # Still running
            ...
            if final_status["running"]:
                ...
                console.print("[red]❌ Server failed to start (no processes found)[/red]")
                raise typer.Exit(1)
        else:
...
def stop_mcp_server() -> bool:
    ...
    for proc in processes:
        try:
            ...
            try:
                proc.wait(timeout=5)
                console.print(f"[green]✅ Process {proc.pid} terminated gracefully[/green]")
                stopped_count += 1
            except psutil.TimeoutExpired:
                # Force kill if graceful shutdown failed
                console.print(f"[yellow]⚡ Force killing process {proc.pid}...[/yellow]")
                proc.kill()
                proc.wait()
...
def show_config() -> None:
    ...
    try:
        ...
        if pyproject_path.exists():
            ...
            config_table.add_row("WebSocket Port", str(config.get("websocket_monitor_port", 8677)))
...
def main(
    ...
    if restart_mcp_server:
        restart_mcp_server_func(port=port, websocket_port=websocket_port, verbose=verbose)
        return
    ...
    console.print("[yellow]No command specified. Use --help for available options.[/yellow]")
