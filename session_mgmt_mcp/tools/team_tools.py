...
def register_team_tools(mcp) -> None:
    ...
    async def create_team_user(
        ...
        try:
            from ..team_knowledge import TeamKnowledgeManager
            ...
            return f"❌ Failed to create team user: {str(e)}"
    ...
    async def add_team_reflection(
        ...
        try:
            ...
            logger.error(f"Team reflection addition failed: {e}")
            return f"❌ Failed to add team reflection: {str(e)}"
    ...
    async def search_team_knowledge(
        ...
        try:
            ...
            logger.error(f"Team knowledge search failed: {e}")
            return f"❌ Team knowledge search failed: {str(e)}"
    ...
    async def join_team(user_id: str, team_id: str, requester_id: str | None = None) -> str:
        ...
        try:
            ...
            logger.error(f"Team join failed: {e}")
            return f"❌ Failed to join team: {str(e)}"
    ...
    async def get_team_statistics(team_id: str, user_id: str) -> str:
        ...
        try:
            ...
            logger.error(f"Team statistics failed: {e}")
            return f"❌ Failed to get team statistics: {str(e)}"
    ...
    async def get_user_team_permissions(user_id: str) -> str:
        ...
        try:
            ...
            output += f"- Can add reflections: {perms.get('can_add_reflections', True)}\n"
            ...
            if stats:
                output += f"\n**User Statistics**:\n"
                output += f"- Total reflections: {stats.get('reflections_created', 0)}\n"
    ...
    async def vote_on_reflection(
        ...
        try:
            ...
            vote_action = "upvoted" if vote_delta > 0 else "downvoted" if vote_delta < 0 else "neutral"
            ...
            if result.get('previous_vote'):
                output += f"ℹ️ Updated previous vote"
            else:
                output += f"ℹ️ First vote on this reflection"
            ...
            return f"❌ Vote failed: {str(e)}"
        except Exception as e:
            logger.error(f"Voting failed: {e}")
            return f"❌ Failed to vote on reflection: {str(e)}"
    ...
    async def list_team_projects(team_id: str, user_id: str) -> str:
        ...
        try:
            ...
            return f"❌ Failed to list team projects: {str(e)}"

    @mcp.tool()
    async def get_team_activity_feed(team_id: str, user_id: str, limit: int = 20) -> str:
        ...
        try:
            ...
            return f"❌ Failed to get activity feed: {str(e)}"