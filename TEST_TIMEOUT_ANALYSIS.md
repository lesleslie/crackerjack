...
2. Add benchmark markers to performance tests
3. Reduce dummy task sleeps 10s→0.01s
4. Reduce timeout test sleeps 1s→0.1s
5. Reduce profiler integration sleeps

### Sprint 2: Collection Optimization (4 hours, 50s saved)
1. Profile pytest collection phase
2. Lazy-load heavy fixtures
3. Optimize import chains
4. Defer DI container initialization

### Sprint 3: Advanced Optimizations (8 hours, 20s saved)
1. Implement mock time for timeout tests
2. Benchmark-specific test isolation
3. Fixture caching strategies
...
2. `tests/performance/test_triple_parallelism_benchmarks.py` - Add markers
3. `tests/unit/core/test_resource_manager.py` - Reduce sleeps (lines 421, 437, 452, 565)
4. `tests/unit/core/test_timeout_manager.py` - Reduce sleeps (~20 locations)
5. `tests/test_profiler_integration.py` - Reduce sleeps (lines 24, 27, 31, 34, 92)
6. `tests/test_core_performance.py` - Add benchmark marker

### Medium Priority:
7. `pyproject.toml` - Add marker configuration
8. `.github/workflows/*.yml` - Update CI to skip benchmarks
9. Fixture optimization in `conftest.py` files
