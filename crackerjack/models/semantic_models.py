...
class EmbeddingVector(BaseModel):
    ...
    embedding: list[float] = Field(..., description="The numerical vector representation")
    created_at: datetime = Field(default_factory=datetime.now, description="Creation timestamp")
    file_hash: str = Field(..., description="Hash of the source file for change detection")
...
class SearchQuery(BaseModel):
    ...
    file_types: list[str] = Field(default_factory=list, description="Filter by file types")
    include_context: bool = Field(default=True, description="Include surrounding context lines")
    context_lines: int = Field(default=3, ge=0, le=10, description="Number of context lines")
...
class IndexingProgress(BaseModel):
    ...
    estimated_remaining: float | None = Field(default=None, description="Estimated remaining time in seconds")
...
class SemanticConfig(BaseModel):
    ...
    chunk_size: int = Field(default=500, ge=100, le=2000, description="Maximum characters per chunk")
    chunk_overlap: int = Field(default=50, ge=0, le=500, description="Overlap between chunks")
    max_search_results: int = Field(default=10, ge=1, le=100, description="Maximum number of search results")
    similarity_threshold: float = Field(default=0.7, ge=0.0, le=1.0, description="Minimum similarity threshold")
    embedding_dimension: int = Field(default=384, ge=128, le=1024, description="Embedding vector dimension")
    max_file_size_mb: int = Field(default=10, ge=1, le=100, description="Maximum file size to process")
    ...
    cache_ttl_hours: int = Field(default=24, ge=1, le=168, description="Cache time-to-live in hours")
...
class FileChangeEvent(BaseModel):
    ...
    event_type: t.Literal["created", "modified", "deleted"] = Field(..., description="Type of change")
    timestamp: datetime = Field(default_factory=datetime.now, description="When the change occurred")
    file_hash: str | None = Field(default=None, description="New file hash if available")
...
class SemanticContext(BaseModel):
    ...
    related_files: list[SearchResult] = Field(..., description="Semantically related files")
    patterns: list[str] = Field(default_factory=list, description="Identified code patterns")
    suggestions: list[str] = Field(default_factory=list, description="AI-generated suggestions")
    confidence: float = Field(..., ge=0.0, le=1.0, description="Confidence in the context relevance")
