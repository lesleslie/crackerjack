"""Deterministic refurb (FURB-code) AST/regex transform logic.

Extracted from ``crackerjack.agents.refurb_agent.RefurbCodeTransformerAgent``
(2,146 lines -- the largest single extraction in this plan). The original
class implements a three-tier fix strategy per ``Issue``:

1. An AST-based rewrite (one of the ten ``_ast_transform_*`` methods), tried
   first, but only for the ten FURB codes that have one.
2. A regex-based rewrite (one of the fifty-four ``_transform_*`` methods
   named in ``FURB_TRANSFORMATIONS``), tried if (1) didn't change anything --
   this is the *only* handler for the other forty-four codes.
3. ``SafeRefurbFixer`` (``crackerjack.services.refurb_fixer``), tried as a
   last resort across the whole file regardless of FURB code.

Scope note: the task brief's "Interfaces" section names only the
``_ast_transform_*`` methods as the deliverable ("plain functions, one per
``_ast_transform_*`` method"). In practice the AST transforms are a
preferred-path *overlay* covering only 10 of the 54 dispatched FURB codes --
they are not usable on their own to reproduce the agent's fix behavior for
the other 44 codes, which have no AST handler at all and rely entirely on
their ``_transform_*`` regex method. Porting only the 10 AST methods would
have produced a module that silently drops fix capability for 44/54 FURB
codes. All 54 dispatch-mapped ``_transform_*`` methods are ported here too,
alongside the 10 ``_ast_transform_*`` methods, matching the precedent set by
earlier tasks in this plan (e.g. Task 4's port of ``RefactoringAgent``'s full
helper-class logic, not just the brief's single named example).

What was intentionally dropped versus the original
``RefurbCodeTransformerAgent`` (``SubAgent``/coordinator plumbing):

- ``__init__`` -- only set ``self.log = logger.info``, DI wiring for the
  deleted ``SubAgent`` framework.
- ``get_supported_types``, ``can_handle`` -- issue-routing confidence
  scoring for the coordinator's dispatch table; this module doesn't decide
  *whether* to run, only *how* to transform content once called.
- ``analyze_and_fix`` -- the top-level orchestrator. It read the file via
  ``AgentContext.get_file_content``, ran the two-tier canonical fix (AST
  then regex) via ``_try_ast_transform``/the resolved handler, wrote the
  result via ``AgentContext.write_file_content``, re-read the file to
  verify the write actually landed (the "ghost-fix guard"), and fell back
  to ``SafeRefurbFixer._apply_fixes`` across the whole file if the
  canonical fix didn't apply. All of the file I/O, write-verification, and
  ``SafeRefurbFixer`` fallback is ``AgentContext``/service-coupled
  orchestration with no independent transform value, and is dropped. The
  *dispatch* core (extract FURB code -> resolve handler -> try AST, fall
  back to regex) is real, reusable logic and is kept below as
  ``fix_refurb_issue`` -- same two-tier resolution, pure content-in/
  content-out, no file I/O, no ``SafeRefurbFixer``.
- ``agent_registry.register(RefurbCodeTransformerAgent)`` -- registry
  plumbing.
- ``SafeRefurbFixer`` (``crackerjack/services/refurb_fixer.py``) -- this is
  already a separate, independent service (not part of ``agents/``), not an
  agent method, and out of scope for this extraction. Not imported here.

Note for Task 22a (the consolidated ``cwd=project_path`` fix, tracked
separately): this file has no subprocess-invoking helper at all --
``RefurbCodeTransformerAgent`` never calls ``SubAgent.run_command`` or
``subprocess`` directly (the ``FURB190``/``_transform_subprocess_list`` name
refers to a regex rewrite *about* lambda-wrapped str methods, not a
subprocess invocation). Nothing to add to Task 22a's file list from this
extraction.

Several pre-existing behavioral quirks are preserved verbatim below, not
"fixed," per CLAUDE.md Rule 7 ("preserve functional requirements... fix the
technical issue, not the requirements"):

1. ``_try_ast_transform``'s fourth parameter, ``handler``, is accepted but
   never used in its body -- the function always rebuilds its own internal
   ``ast_handlers`` dict (keyed by ``furb_code``) from scratch and ignores
   whatever ``handler`` object the caller passed in. ``fix_refurb_issue``
   below still passes a ``handler`` argument through, matching the original
   call shape and the existing test suite's direct calls to
   ``_try_ast_transform(content, issue, furb_code, handler)``, even though
   it has no effect.
2. ``_unparse_tree``'s second parameter, ``original_content``, is accepted
   but never read in its body.
3. ``_transform_write_whole_file`` and ``_transform_substring`` are
   orphaned: both are fully implemented and directly unit-tested in the
   original test suite, but neither appears anywhere in
   ``FURB_TRANSFORMATIONS``, so neither is ever reachable through
   ``fix_refurb_issue``'s (or the original ``analyze_and_fix``'s) dispatch.
   Ported as-is, callable directly, unreachable via dispatch -- exactly
   matching the original.
4. ``_transform_print_literal`` is a one-line delegate
   (``return _transform_list_comprehension(content, issue)``) that is
   *also* orphaned -- absent from ``FURB_TRANSFORMATIONS`` and, unlike the
   two above, not directly tested anywhere in the original suite either.
   Ported anyway for maximum fidelity (zero behavioral cost either way).
5. ``_transform_max_min`` contains a genuine no-op statement:
   ``if new_content != content and "enumerated-as-direct" not in fixes[-1:] if fixes else True: pass``
   -- a nested conditional expression that computes a boolean and then does
   nothing with it. Preserved verbatim.
6. Most ``_transform_*`` regex handlers accept an ``issue: Issue`` parameter
   that their body never reads (the regex patterns operate on ``content``
   alone). Preserved for uniform dispatch -- ``fix_refurb_issue`` calls
   every resolved handler the same way, ``handler(content, issue)``.
7. ``_extract_furb_code`` checks ``hasattr(issue, "reason") and issue.reason``.
   ``crackerjack.models.issues.Issue`` (like the original
   ``crackerjack.agents.base.Issue``) has no ``reason`` field, so this
   branch is unreachable for a real ``Issue`` instance and only fires in
   tests that explicitly stub a ``.reason`` attribute onto a ``Mock``.
   Preserved verbatim (defensive/dead code in production, live in tests).
"""
...
def _extract_furb_code(issue: Issue) -> str | None:
...
def _try_ast_transform(
    content: str, issue: Issue, furb_code: str, handler: object
) -> tuple[str, str]:
    try:
        tree = ast.parse(content)
        line_number = issue.line_number or 0
        ast_handlers = {
            "FURB107": _ast_transform_suppress,
            "FURB102": _ast_transform_startswith_tuple,
            "FURB109": _ast_transform_membership_tuple,
            "FURB118": _ast_transform_itemgetter,
            "FURB126": _ast_transform_remove_else_return,
            "FURB110": _ast_transform_or_operator,
            "FURB115": _ast_transform_len_comparison,
            "FURB113": _ast_transform_append_to_extend,
            "FURB123": _ast_transform_list_copy,
            "FURB142": _ast_transform_set_update,
        }
        ast_handler = ast_handlers.get(furb_code)
        if ast_handler:
            new_tree, fix_desc = ast_handler(tree, line_number, content)
            if new_tree:
                new_content = _unparse_tree(new_tree, content)
...
def _unparse_tree(tree: ast.AST, original_content: str) -> str | None:
...
def _ast_transform_suppress(
    tree: ast.AST, line_number: int, content: str
    ...
    new_content = _ensure_import_after_others(
...
def _ast_transform_startswith_tuple(
    ...
    new_content, pattern_label = _ast_line_rewrite(
        content,
        line_number,
        [
            (
...
def _ast_transform_membership_tuple(
    ...
    new_content, _ = _ast_line_rewrite(
        content,
        line_number,
...
def _ast_transform_itemgetter(
    ...
    new_content = _ensure_import_after_others(new_content, "import operator")
    try:
        return (
            ast.parse(new_content),
...
def _ast_transform_len_comparison(
    ...
    new_content, label = _ast_line_rewrite(
        content,
        line_number,
        [
...
def _ast_transform_append_to_extend(
    ...
    replacement = (
        f"{first_indent}{first_var}.extend(({first_arg.strip()}, {second_arg.strip()}))"
    )
...
def _ensure_import_after_others(content: str, import_line: str) -> str:
...
def _transform_enumerate(content: str, issue: Issue) -> tuple[str, str]:
    fixes = []
    new_content = content
...
def _transform_any_all(content: str, issue: Issue) -> tuple[str, str]:
    fixes: list[str] = []
...
def _transform_bool_return(content: str, issue: Issue) -> tuple[str, str]:
    fixes: list[str] = []
...
def _transform_zip(content: str, issue: Issue) -> tuple[str, str]:
    fixes = []
...
def _transform_unnecessary_listcomp(content: str, issue: Issue) -> tuple[str, str]:
    fixes: list[str] = []
...
def _transform_copy(content: str, issue: Issue) -> tuple[str, str]:
    fixes = []
    pattern = "(\\w+)\\[\\s*:\\s*\\](?!\\s*, )"
    replacement = "\\1.copy()"
    new_content = re.sub(pattern, replacement, content)
...
def _transform_max_min(content: str, issue: Issue) -> tuple[str, str]:
    fixes: list[str] = []
...
def _transform_pow_operator(content: str, issue: Issue) -> tuple[str, str]:
    fixes: list[str] = []
    ...
    for pattern_str, replacement in replacements:
        ...
        if count > 0:
            ...
            fixes.append(f"Replaced {count} hardcoded constant(s) with {replacement}")
    return (
        new_content,
...
def _transform_int_scientific(content: str, issue: Issue) -> tuple[str, str]:
    fixes: list[str] = []

    pattern = re.compile(r"\b(bin|oct|hex)\s*\(([^)]+)\)\.count\(\s*['\"]1['\"]\s*\)")
...
def _transform_sorted_key_identity(content: str, issue: Issue) -> tuple[str, str]:
    fixes: list[str] = []
...
def _transform_compare_zero(content: str, issue: Issue) -> tuple[str, str]:
    fixes = []
    ...
    new_content = re.sub(not_endswith_pattern, "not \\1.endswith((\\2, \\3))", content)
    if new_content != content:
...
def _transform_compare_empty(content: str, issue: Issue) -> tuple[str, str]:
    fixes = []
...
def _transform_redundant_none_comparison(content: str, issue: Issue) -> tuple[str, str]:
    fixes: list[str] = []
    _val = r"""(?:'[^']*'|"[^"]*"|\w+)"""
    pattern = re.compile(
...
def _transform_membership_test(content: str, issue: Issue) -> tuple[str, str]:
    fixes = []
    pattern = "\\bin\\s*\\[([^\\]]+)\\]"
    replacement = "in (\\1)"
    new_content = re.sub(pattern, replacement, content)
...
def _transform_isinstance_type_check(content: str, issue: Issue) -> tuple[str, str]:
    fixes: list[str] = []
    new_content = content
...
def _transform_write_whole_file(content: str, issue: Issue) -> tuple[str, str]:
    fixes = []
    pattern = (
...
def _transform_multiple_with(content: str, issue: Issue) -> tuple[str, str]:
    fixes = []
    pattern = "with\\s+([^:]+):\\s*\\n\\s*with\\s+([^:]+):"
...
def _transform_redundant_not(content: str, issue: Issue) -> tuple[str, str]:
    fixes: list[str] = []
...
def _transform_substring(content: str, issue: Issue) -> tuple[str, str]:
    fixes = []
...
def _transform_useless_fstring(content: str, issue: Issue) -> tuple[str, str]:
    fixes = []
...
def _transform_print_empty_string(content: str, issue: Issue) -> tuple[str, str]:
    fixes = []
    new_content = content
...
def _transform_delete_while_iterating(content: str, issue: Issue) -> tuple[str, str]:

    pattern = re.compile(r"(\b\w[\w.]*\s*=\s*)([\w.]+)\s+if\s+\2\s+else\s+([\w.]+)\b")
...
def _transform_redundant_continue(content: str, issue: Issue) -> tuple[str, str]:
    fixes = []
    pattern = "\\n(\\s*)continue\\s*\\n(\\s*)\\n"
    replacement = "\\n\\2\\n"
    new_content = re.sub(pattern, replacement, content)
...
def _transform_list_copy(content: str, issue: Issue) -> tuple[str, str]:
    fixes: list[str] = []
...
def _transform_redundant_pass(content: str, issue: Issue) -> tuple[str, str]:
    fixes = []
    lines = content.split("\n")
...
def _transform_open_mode_r(content: str, issue: Issue) -> tuple[str, str]:
    fixes = []
    new_content = content
...
def _transform_fstring_numeric_literal(content: str, issue: Issue) -> tuple[str, str]:
    import ast as _ast
...
def _transform_redundant_index(content: str, issue: Issue) -> tuple[str, str]:
    fixes: list[str] = []
    _var = r"([\w.]+)"
    transforms = [
        (
...
def _transform_rhs_unpack(content: str, issue: Issue) -> tuple[str, str]:
    fixes: list[str] = []
    pattern = re.compile(
...
def _transform_redundantenumerate(content: str, issue: Issue) -> tuple[str, str]:
    import ast as _ast
...
def _transform_single_item_membership(content: str, issue: Issue) -> tuple[str, str]:
    fixes: list[str] = []
    new_content = content
...
def _transform_check_and_remove(content: str, issue: Issue) -> tuple[str, str]:
    fixes: list[str] = []
    pattern = re.compile(
...
def _transform_bad_open_mode(content: str, issue: Issue) -> tuple[str, str]:
    import ast as _ast
...
def _transform_list_multiply(content: str, issue: Issue) -> tuple[str, str]:
    fixes: list[str] = []
...
def _transform_print_literal(content: str, issue: Issue) -> tuple[str, str]:
    return _transform_list_comprehension(content, issue)


def _transform_list_comprehension(content: str, issue: Issue) -> tuple[str, str]:
    ...
    target_for = _find_list_comprehension_loop(tree, issue.line_number)
    if target_for is None:
        return (content, "No list comprehension loop found")

    extract_result = _build_list_comprehension_rewrite(content, target_for)
    if extract_result is None:
        return (content, "List comprehension pattern requires a simple loop body")
    return extract_result


def _find_list_comprehension_loop(tree: ast.AST, line_number: int) -> ast.For | None:
...
def _build_list_comprehension_rewrite(
    content: str, target_for: ast.For
) -> tuple[str, str] | None:
    if not target_for.body or len(target_for.body) > 2:
        return None

    append_stmt, assign_stmt = _extract_append_loop_parts(target_for)
    if append_stmt is None:
        return None

    list_name = _get_append_target_name(append_stmt)
    if list_name is None:
        return None

    item_expr = _get_list_comprehension_item_expr(
        content,
        append_stmt,
        assign_stmt,
    )
    ...
    loop_signature = _get_list_comprehension_loop_signature(
        content,
        target_for,
        list_name,
        item_expr,
    ...
    new_content = _rewrite_loop_as_list_comprehension(
        content,
        target_for,
        list_name,
        item_expr,
...
def _get_append_target_name(append_stmt: ast.Expr) -> str | None:
    ...
    if len(call_value.args) != 1:  # type: ignore[union-attr]
        return None
    return call_value.func.value.id
...
def _get_list_comprehension_item_expr(
    ...
    item_expr = ast.get_source_segment(content, append_arg) or ast.unparse(append_arg)
    ...
    if (
        ...
        item_expr = ast.get_source_segment(content, assign_stmt.value) or ast.unparse(
            assign_stmt.value
        )
...
def _get_list_comprehension_loop_signature(
    ...
    target_source = ast.get_source_segment(content, target_for.target) or ast.unparse(
        target_for.target
    )
    iter_source = ast.get_source_segment(content, target_for.iter) or ast.unparse(
        target_for.iter
    )
    init_start = _find_list_initialization_line(content, target_for, list_name)
...
def _find_list_initialization_line(
    content: str, target_for: ast.For, list_name: str
) -> int | None:
    lines = content.split("\n")
...
def _transform_redundant_fstring(content: str, issue: Issue) -> tuple[str, str]:
    fixes: list[str] = []
    ...
    if (
        fixes
        and "from pathlib import Path" not in new_content
        and (
...
def _transform_no_default_or(content: str, issue: Issue) -> tuple[str, str]:
...
def _transform_redundant_lambda(content: str, issue: Issue) -> tuple[str, str]:
    fixes: list[str] = []
...
def _transform_implicit_print(content: str, issue: Issue) -> tuple[str, str]:
    fixes: list[str] = []
...
def _transform_dict_literal(content: str, issue: Issue) -> tuple[str, str]:
    _flag_map = {
        "I": "IGNORECASE",
        "M": "MULTILINE",
        "S": "DOTALL",
...
def _transform_isinstance_type_tuple(content: str, issue: Issue) -> tuple[str, str]:
    fixes: list[str] = []
...
def _transform_type_none_comparison(content: str, issue: Issue) -> tuple[str, str]:
    fixes: list[str] = []
...
def _transform_single_element_membership(content: str, issue: Issue) -> tuple[str, str]:
...
def _transform_unnecessary_list_cast(content: str, issue: Issue) -> tuple[str, str]:
    fixes: list[str] = []
    pattern = re.compile(r"\b([\w.]+)\.name\.endswith\s*\(\s*(['\"][^'\"]+['\"])\s*\)")
...
def _transform_abs_sqr(content: str, issue: Issue) -> tuple[str, str]:
    fixes: list[str] = []
    _val = r"""(?:None|\.\.\.|"[^"]*"|'[^']*'|-?\d+(?:\.\d+)?)"""
    pattern = re.compile(
...
def _transform_unnecessary_from_float(content: str, issue: Issue) -> tuple[str, str]:
    fixes: list[str] = []
    ...
    result = utcfromts_pat.sub(r"datetime.fromtimestamp(\1, timezone.utc)", new_content)
    if result != new_content:
        fixes.append(
...
def _transform_redundant_or(content: str, issue: Issue) -> tuple[str, str]:
    fixes: list[str] = []
...
def _transform_method_assign(content: str, issue: Issue) -> tuple[str, str]:
    fixes: list[str] = []
...
def _transform_redundant_expression(content: str, issue: Issue) -> tuple[str, str]:
    fixes: list[str] = []
...
def _transform_bad_version_info_compare(content: str, issue: Issue) -> tuple[str, str]:
    fixes: list[str] = []
    pattern = re.compile(
        r"^(\s*)(\w+)\s*=\s*\2\.(\w+\([^)\n]*\))\s*\n"
        r"\s*\2\s*=\s*\2\.(\w+\([^)\n]*\))\s*$",
...
def _transform_redundant_substring(content: str, issue: Issue) -> tuple[str, str]:
    fixes: list[str] = []
    pattern = re.compile(
        r"^(\s*)(\w+)\s*=\s*([\w.]+)\.copy\(\)\s*\n"
...
def _transform_redundant_cast(content: str, issue: Issue) -> tuple[str, str]:
    fixes: list[str] = []
    pattern = re.compile(
...
def _transform_chained_assignment(content: str, issue: Issue) -> tuple[str, str]:
    fixes: list[str] = []
...
def _transform_slice_copy(content: str, issue: Issue) -> tuple[str, str]:
    fixes: list[str] = []
    ...
    for m in list(prefix_pattern.finditer(content)):
        ...
        new_content = new_content.replace(m.group(0), replacement, 1)
        fixes.append(f"Rewrote startswith+slice as removeprefix({quote}{lit}{quote})")
...
def _transform_fstring_to_print(content: str, issue: Issue) -> tuple[str, str]:
...
def _transform_subprocess_list(content: str, issue: Issue) -> tuple[str, str]:
    fixes: list[str] = []
    str_methods = (
...
def fix_refurb_issue(content: str, issue: Issue) -> tuple[str, str]:
    """Apply the FURB-code fix for ``issue`` to ``content``.

    Mirrors the "canonical fix" branch of the original
    ``RefurbCodeTransformerAgent.analyze_and_fix``: extract the FURB code
    from ``issue``, resolve its ``_transform_*`` handler via
    ``FURB_TRANSFORMATIONS``, try the AST-based rewrite first (only ten
    codes have one), and fall back to the regex-based handler if the AST
    path didn't change anything. Pure content-in/content-out -- no file
    I/O, no write-verification, no ``SafeRefurbFixer`` fallback (see the
    module docstring for what those looked like and why they're dropped).

    Returns ``(content, <reason>)`` unchanged if no FURB code can be
    extracted from ``issue`` or no handler is registered for it.
    """
    furb_code = _extract_furb_code(issue)
    if furb_code is None:
        return content, "No FURB code found on issue"
    handler_name = FURB_TRANSFORMATIONS.get(furb_code)
    handler = globals().get(handler_name) if handler_name else None
    if handler is None:
        return content, f"No handler for {furb_code}"
    new_content, fix_description = _try_ast_transform(
        content, issue, furb_code, handler
    )
    if new_content == content:
        new_content, fix_description = handler(content, issue)
    return new_content, fix_description
