...
class PhaseCoordinator:
    _event_publisher: t.Any | None = None
    ...
    def __init__(
        ...
        *,
        ...
        event_publisher: t.Any | None = None,
    ) -> None:
    ...
    @handle_errors
    def run_coverage_ratchet_phase(self, options: OptionsProtocol) -> bool:

        if getattr(options, "no_coverage_ratchet", False):
            return True
        self.session.track_task("coverage_ratchet", "Coverage ratchet refresh")
        self.console.print(
            "\n" + make_separator("-"),
        )
        self.console.print(
            "[bold bright_blue]📊 RATCHET[/bold bright_blue] "
    ...
    def _process_hook_results(
        ...
        return summary.get("failed", 0) == 0 == summary.get("errors", 0)
    ...
    def _should_update_hook_count(self, result: HookResult) -> bool:
        ...
        return not (
            not result.output or not result.output.strip().startswith(("{", "["))
        )
