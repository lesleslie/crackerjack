...
async def _validate_context_and_rate_limit(context: t.Any) -> str | None:
    ...
    if not context:
        return json.dumps({
            "status": "error",
            "message": "MCP context not available"
        })
...
def _parse_init_arguments(args: str, kwargs: str) -> tuple[t.Any, bool, str | None]:
    ...
    try:
        target_path = args.strip() if args.strip() else "."
...
def _generate_agent_recommendations(
    ...
    suggestions = _analyze_task_for_agents(task_description, project_type, current_context)
...
def _analyze_task_for_agents(
    ...
    if any(keyword in task_lower for keyword in ["security", "vulnerability", "secure"]):
        agents.append({
            "name": "SecurityAgent",
    ...
    if any(keyword in task_lower for keyword in ["performance", "optimize", "speed", "slow"]):
        agents.append({
    ...
    # Documentation tasks
    if any(keyword in task_lower for keyword in ["document", "readme", "doc", "explain"]):
        agents.append({
    ...
    if any(keyword in task_lower for keyword in ["import", "dependency", "package", "module"]):
        agents.append({
