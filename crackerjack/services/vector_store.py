...
class VectorStore:
    ...
    def index_file(self, file_path: Path, progress_callback: t.Callable[[IndexingProgress], None] | None = None) -> list[EmbeddingVector]:
        ...
        try:
            ...
            embedding_vectors = self.embedding_service.generate_embeddings_batch(chunk_texts)

            # Create EmbeddingVector objects
            for i, (embedding_vector, metadata) in enumerate(zip(embedding_vectors, chunk_metadata)):
            ...
            logger.info(f"Successfully indexed {len(embeddings)} chunks from {file_path}")
            return embeddings
    ...
    def _update_file_tracking(self, file_path: Path, file_hash: str, chunk_count: int) -> None:
    ...
    def _get_all_embeddings(self, file_types: list[str] | None = None) -> list[dict[str, t.Any]]:
    ...
    def get_stats(self) -> IndexStats:
        ...
        with self._get_connection() as conn:
            ...
            cursor = conn.execute("SELECT COUNT(DISTINCT file_path) as total_files FROM embeddings")
            ...
            cursor = conn.execute("SELECT MAX(created_at) as last_updated FROM embeddings")
            ...
            cursor = conn.execute("SELECT AVG(LENGTH(content)) as avg_size FROM embeddings")
            avg_chunk_size = cursor.fetchone()["avg_size"] or 0.0

            # Estimate index size (rough approximation)
            index_size_mb = (total_chunks * 384 * 4) / (1024 * 1024)  # Assuming 384-dim embeddings
    ...
    def remove_file(self, file_path: Path) -> bool:
        ...
        with self._get_connection() as conn:
            ...
            conn.execute("DELETE FROM file_tracking WHERE file_path = ?", (str(file_path),))
