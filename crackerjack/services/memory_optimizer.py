        ...
        with self._lock:
            if not self._loaded:
                ...
                try:
                    ...
                    self._logger.debug(
                        f"Loaded {self._name} in {load_time:.3f}s"
                    )
        ...
        with self._lock:
            if resource in self._in_use:
                ...
                if len(self._pool) < self._max_size:
                    ...
                    self._logger.debug(f"Pool full, disposed resource from {self._name}")
...
class MemoryOptimizer:
    ...
    def __init__(self):
        self._lazy_objects: WeakSet = WeakSet()
...
def lazy_property(factory: Callable[[], T]) -> Callable[[Any], T]:
...
def create_lazy_service(factory: Callable[[], T], name: str) -> LazyLoader[T]:
