...
class ExecutionStrategy(Enum):
    ...
    PARALLEL_SAFE = "parallel_safe"  # Only parallel execution of safe, independent hooks
...
class ParallelHookExecutor:
    ...
    def can_execute_in_parallel(
        ...
        safe_parallel_combinations = [
            ...
            (
                lambda h: not h.is_formatting and h.security_level == SecurityLevel.MEDIUM,
                lambda h: not h.is_formatting and h.security_level == SecurityLevel.MEDIUM,
            ),
        ]
    ...
    async def _execute_group_parallel(
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                ...
                self._logger.error(f"Hook {hooks[i].name} failed with exception: {result}")
            else:
...
class AsyncCommandExecutor:
    ...
    async def execute_command(
        ...
        if self.cache_results:
            ...
            if cached_result:
                self._logger.debug(f"Using cached result for command: {' '.join(command)}")
        ...
        if self.cache_results and result.success:
            await self._cache_result(command, cwd, result, cache_ttl)
    ...
    async def execute_commands_batch(
        ...
        tasks = [
            self.execute_command(cmd, cwd, timeout)
        ...
        successful = sum(1 for r in processed_results if r.success)
        self._logger.info(f"Batch execution: {successful}/{len(commands)} commands succeeded")
