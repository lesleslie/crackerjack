...
GIT_COMMANDS = {'git_dir': ['rev-parse', '--git-dir'], 'staged_files': ['diff', '--cached', '--name-only', '--diff-filter=ACMRT'], 'unstaged_files': ['diff', '--name-only', '--diff-filter=ACMRT'], 'untracked_files': ['ls-files', '--others', '--exclude-standard'], 'staged_files_simple': ['diff', '--cached', '--name-only'], 'add_file': ['add'], 'add_all': ['add', '-A', '.'], 'commit': ['commit', '-m'], 'add_updated': ['add', '-u'], 'push_porcelain': ['push', '--porcelain'], 'push_with_tags': ['push', '--porcelain', '--follow-tags'], 'current_branch': ['branch', '--show-current'], 'commits_ahead': ['rev-list', '--count', '@{u}..HEAD']}
...
class GitService(GitInterface):
    AUTH_ERROR_PATTERNS = ['Permission denied', 'publickey', 'Authentication failed', 'Could not read from remote repository', 'fatal: unable to access', '401', '403', 'Remote: Invalid username or password', 'fatal: Authentication failed']

    def __init__(self, console: ConsoleInterface | Path | None=None, pkg_path: Path | None=None, auth_fallback: bool=True, persist_fallback: bool=False) -> None:
    ...
    def _run_git_command(self, args: list[str]) -> subprocess.CompletedProcess[str] | FailedGitResult:
        cmd = ['git', *args]
        try:
            return execute_secure_subprocess(command=cmd, cwd=self.pkg_path, capture_output=True, text=True, timeout=60, check=False)
        except Exception as e:
            security_logger = get_security_logger()
            security_logger.log_subprocess_failure(command=cmd, exit_code=-1, error_output=str(e))
        ...
        try:
            for file in files:
                ...
                if result.returncode != 0:
                    self.console.print(f'[red]❌[/ red] Failed to add {file}: {result.stderr}')
                    return False
            return True
        except Exception as e:
    ...
    def add_all_files(self) -> bool:
        try:
            ...
            if result.returncode == 0:
                ...
                return True
            self.console.print(f'[red]❌[/ red] Failed to stage changes: {result.stderr}')
            return False
        except Exception as e:
    ...
    def _handle_commit_failure(self, result: subprocess.CompletedProcess[str] | FailedGitResult, message: str) -> bool:
    ...
    def _retry_commit_after_restage(self, message: str) -> bool:
        self.console.print('[yellow]🔄[/yellow] Hooks modified files - attempting to re-stage and retry commit')
        add_result = self._run_git_command(GIT_COMMANDS['add_updated'])
        if add_result.returncode != 0:
            self.console.print(f'[red]❌[/ red] Failed to re-stage files: {add_result.stderr}')
            return False
        cmd = GIT_COMMANDS['commit'] + [message]
        retry_result = self._run_git_command(cmd)
        if retry_result.returncode == 0:
            self.console.print(f'[green]✅[/ green] Committed after re-staging: {message}')
            return True
        self.console.print(f'[red]❌[/ red] Commit failed on retry: {retry_result.stderr}')
        return False

    def _handle_hook_error(self, result: subprocess.CompletedProcess[str] | FailedGitResult) -> bool:
        if 'pre-commit' in result.stderr or 'hook' in result.stderr.lower():
            self.console.print('[red]❌[/ red] Commit blocked by hooks')
            if result.stderr.strip():
                self.console.print(f'[yellow]Hook output: [/ yellow]\n{result.stderr.strip()}')
        else:
    ...
    def _try_auth_fallback(self) -> bool:
        ...
        if original_url.startswith('git@'):
            ...
            return False
        self.console.print(f'[yellow]🔄[/ yellow] Auth failed, trying fallback ({auth_type})...')
        ...
        if result.returncode == 0:
            self._display_push_success(result.stdout)
            self.console.print(f'[green]✅[/ green] Push succeeded using fallback auth ({auth_type})')
            if not self.persist_fallback:
                self._set_remote_url(original_url)
                self.console.print('[dim]💡 Tip: Set git.persist_fallback=true to remember this auth method[/ dim]')
            else:
                self.console.print(f"[green]📌[/ green] Updated remote URL to use {fallback_url.split(':')[0]}")
            return True
        self._set_remote_url(original_url)
    ...
    def _display_push_results(self, pushed_refs: list[str]) -> None:
        if pushed_refs:
            self.console.print(f'[green]✅[/ green] Successfully pushed {len(pushed_refs)} ref(s) to remote: ')
    ...
    def _display_commit_count_push(self) -> None:
        try:
            ...
            if result.returncode == 0 and result.stdout.strip().isdigit():
                ...
                if commit_count > 0:
                    self.console.print(f'[green]✅[/ green] Pushed {commit_count} commit(s) to remote')
                else:
                    self.console.print('[green]✅[/ green] Pushed to remote (up to date)')
            else:
    ...
    def _categorize_files(self, files: list[str]) -> set[str]:
        categories = {'docs': ['README', 'CLAUDE', 'docs /', '.md'], 'tests': ['test_', 'tests /', 'conftest.py'], 'config': ['pyproject.toml', '.yaml', '.yml', '.json', '.gitignore'], 'ci': ['.github /', 'ci /', '.pre-commit'], 'deps': ['requirements', 'uv.lock', 'Pipfile']}
    ...
    def _generate_single_category_message(self, category: str) -> list[str]:
        category_messages = {'docs': 'Update documentation', 'tests': 'Update tests', 'config': 'Update configuration', 'ci': 'Update CI / CD configuration', 'deps': 'Update dependencies'}
    ...
    def get_changed_files_by_extension(self, extensions: list[str], include_staged: bool=True, include_unstaged: bool=True) -> list[Path]:
        try:
            ...
            filtered = [self.pkg_path / f for f in all_changed if f and any((f.endswith(ext) for ext in extensions))]
            return [f for f in filtered if f.exists()]
        except Exception as e:
            self.console.print(f'[yellow]⚠️[/yellow] Error getting changed files by extension: {e}')
            return []
    ...
    def get_changed_files_since(self, since: str, project_root: Path) -> list[str]:
        try:
            result = self._run_git_command(['diff', '--name-only', since, 'HEAD'])
            if result.returncode == 0:
                return [str(project_root / f) for f in result.stdout.strip().split('\n') if f.strip()]
            return []
        except Exception:
            return []
    ...
    def get_unstaged_files(self) -> list[str]:
        try:
            ...
            if result.returncode == 0:
                return [str(self.pkg_path / f) for f in result.stdout.strip().split('\n') if f.strip()]
            return []
        except Exception:
            return []