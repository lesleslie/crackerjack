...
def run(
    ...
    publish_url: str | None = CLI_OPTIONS["publish_url"],
    publish_token_env: str | None = CLI_OPTIONS["publish_token_env"],
    ...
    fail_first: bool = CLI_OPTIONS["fail_first"],
...
def _print_run_context() -> None:
    ...
    if venv_name:
        console.print(f"[dim]venv:[/dim] {venv_name}")
...
def _create_and_configure_options(local_vars: dict[str, t.Any]) -> Options:
    options = create_options(
        ...
        publish_url=local_vars["publish_url"],
        publish_token_env=local_vars["publish_token_env"],
        ...
        fail_first=local_vars["fail_first"],
