...
This module provides functionality to load and manage configuration profiles.
Profiles are predefined configurations that optimize Crackerjack for specific use cases.

Profiles:
- quick: Fast development feedback (< 1 minute)
- standard: Balanced pre-commit checks (2-5 minutes)
...
class ProfileLoader:
    ...
    Example:
        ...
        >>> profile = loader.load_profile("standard")
        >>> print(profile.profile.description)
        Balanced checks for pre-commit and CI (2-5 minutes)
    """
    ...
    def __init__(self, profile_dir: Path | None = None):
        ...
        Args:
            profile_dir: Directory containing profile YAML files.
                         Defaults to settings/profiles/ relative to package root.
        """
        if profile_dir is None:
            # Default to settings/profiles/ in the package
    ...
    def load_profile(self, profile_name: str) -> ProfileConfig:
        ...
        try:
            ...
            # Validate and create ProfileConfig
    ...
    def get_profile_metadata(self, profile_name: str) -> ProfileMetadata:
        ...
        Example:
            >>> loader = ProfileLoader()
            >>> meta = loader.get_profile_metadata("quick")
            >>> meta.execution_time
            '< 1 minute'
        """
    ...
    def profile_exists(self, profile_name: str) -> bool:
        ...
        Example:
            >>> loader = ProfileLoader()
            >>> loader.profile_exists("standard")
            True
            >>> loader.profile_exists("nonexistent")
            False
        """
        return (
    ...
    def compare_profiles(self, profile1: str, profile2: str) -> dict[str, Any]:
        ...
        Example:
            >>> loader = ProfileLoader()
            >>> diff = loader.compare_profiles("quick", "comprehensive")
            >>> diff['testing']['enabled']
            {'quick': False, 'comprehensive': True}
        """
...
def load_profile(profile_name: str) -> ProfileConfig:
    """
    Load a profile using the default loader.
...
def list_profiles() -> list[str]:
    """
    List all available profiles using the default loader.
