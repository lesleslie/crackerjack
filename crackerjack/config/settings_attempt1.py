...
class CrackerjackSettings(BaseSettings):
    ...
    instance_id: str | None = Field(
        default=None,
        description="Unique server instance ID for multi-instance support",
    )
    runtime_dir: Path = Field(
        default=Path.home() / ".crackerjack",
    ...
    zuban_enabled: bool = Field(
        default=True,
        description="Enable Zuban ultra-fast type checker",
    )
    skylos_enabled: bool = Field(
        default=True,
        description="Enable Skylos dead code detection",
    )
    pyright_enabled: bool = Field(
        default=False,
        description="Enable Pyright type checking",
    )
    ...
    max_parallel_hooks: int = Field(
        default=4,
        description="Maximum parallel pre-commit hooks",
    )
    ...
    auto_detect_workers: bool = Field(
        default=True,
        description="Automatically detect optimal worker count",
    )
    ...
    memory_per_worker_gb: float = Field(
        default=2.0,
        description="Memory per worker in GB for worker calculation",
    )
    ...
    experimental_hooks: bool = Field(
        default=False,
        description="Enable experimental hooks",
    )
    ...
    keep_debug_logs: int = Field(
        default=5,
        description="Number of debug logs to retain",
    )
    keep_coverage_files: int = Field(
        default=10,
        description="Number of coverage files to retain",
    )
    ...
    zuban_lsp_enabled: bool = Field(
        default=True,
        description="Enable Zuban LSP integration",
    )
    zuban_lsp_auto_start: bool = Field(
        default=True,
        description="Auto-start Zuban LSP server",
    )
    zuban_lsp_port: int = Field(default=8677, description="Zuban LSP server port")
    zuban_lsp_timeout: int = Field(
        default=120,
        description="Zuban LSP timeout in seconds",
    )

    global_lock_enabled: bool = Field(
        default=True,
        description="Enable global lock mechanism",
    )
    global_lock_timeout_seconds: float = Field(
        default=1800.0,
        description="Global lock timeout",
    )
    global_lock_stale_hours: float = Field(
        default=2.0,
        description="Hours before lock is considered stale",
    )
