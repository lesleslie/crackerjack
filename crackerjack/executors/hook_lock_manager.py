...
class HookLockManager:
    ...
    @asynccontextmanager
    async def _acquire_existing_hook_lock(self, hook_name: str) -> t.AsyncIterator[None]:
        ...
        self.logger.debug(f"Acquiring hook-specific lock: {hook_name} (timeout: {timeout}s)")
        ...
        try:
            ...
            try:
                ...
                self.logger.info(f"Hook-specific lock acquired for {hook_name} after {acquisition_time:.2f}s")
                ...
                try:
                    ...
                    self.logger.debug(f"Hook-specific lock released for {hook_name} after {total_time:.2f}s total")
            ...
            self.logger.error(
                f"Hook-specific lock acquisition timeout for {hook_name} after {wait_time:.2f}s "
                f"(timeout: {timeout}s, total failures: {self._timeout_failures[hook_name]})"
            )
            raise
    ...
    @asynccontextmanager
    async def _acquire_global_coordination_lock(self, hook_name: str) -> t.AsyncIterator[None]:
        ...
        self.logger.debug(f"Attempting global lock acquisition for {hook_name}: {lock_path}")
        ...
        try:
            ...
            acquisition_time = time.time() - start_time
            self.logger.info(f"Global lock acquired for {hook_name} after {acquisition_time:.2f}s")
            ...
            self.logger.error(f"Global lock acquisition failed for {hook_name}: {str(e)}")
            raise
        ...
        for attempt in range(self._global_config.max_retry_attempts):
            try:
                ...
                if attempt < self._global_config.max_retry_attempts - 1:
                    ...
                    self.logger.debug(f"Global lock exists for {hook_name}, retrying in {wait_time:.2f}s")
                    await asyncio.sleep(wait_time)
                else:
                    raise asyncio.TimeoutError(f"Failed to acquire global lock for {hook_name} after {self._global_config.max_retry_attempts} attempts")
        ...
        while hook_name in self._active_global_locks:
            try:
                ...
                if self._heartbeat_failures[hook_name] > 3:
                    self.logger.error(f"Too many heartbeat failures for {hook_name}, stopping heartbeat")
                    break

    async def _update_heartbeat_timestamp(self, hook_name: str, lock_path: Path) -> None:
        """Atomic update of heartbeat timestamp in existing lock file."""
        if not lock_path.exists():
            self.logger.warning(f"Lock file disappeared for {hook_name}, stopping heartbeat")
        ...
        try:
            ...
            if lock_data.get("session_id") != self._global_config.session_id:
                self.logger.warning(f"Lock ownership changed for {hook_name}, stopping heartbeat")
                self._active_global_locks.discard(hook_name)
                return
    ...
    async def _cleanup_global_lock(self, hook_name: str, heartbeat_task: asyncio.Task[None]) -> None:
            ...
            if lock_path.exists():
                ...
                try:
                    ...
                    if lock_data.get("session_id") == self._global_config.session_id:
                        ...
                        self.logger.warning(f"Lock ownership changed, not removing file: {lock_path}")
                        
                except Exception as e:
                    self.logger.warning(f"Could not verify lock ownership for cleanup: {e}")
    ...
    async def _cleanup_stale_lock_if_needed(self, hook_name: str) -> None:
        ...
        try:
            ...
            last_heartbeat = lock_data.get("last_heartbeat", lock_data.get("acquired_at", 0))
            age_hours = (time.time() - last_heartbeat) / 3600
            
            if age_hours > self._global_config.stale_lock_hours:
                self.logger.warning(f"Removing stale lock for {hook_name} (age: {age_hours:.2f}h)")
                lock_path.unlink()
                self._stale_locks_cleaned[hook_name] += 1
            else:
                # Lock is not stale, someone else has it
                owner = lock_data.get("session_id", "unknown")
                self.logger.debug(f"Active lock exists for {hook_name} owned by {owner}")
        ...
        for hook_name in self._hooks_requiring_locks:
            ...
            if not usage_list:
                stats[hook_name] = {
                    ...
                    "success_rate": 1.0,
                    "lock_timeout": self._lock_timeouts.get(hook_name, self._default_lock_timeout),
                }
            else:
                total_attempts = len(usage_list) + self._lock_failures[hook_name]
                success_rate = len(usage_list) / total_attempts if total_attempts > 0 else 1.0
    ...
    def enable_global_lock(self, enabled: bool = True) -> None:
        ...
        self.logger.info(f"Global lock functionality {'enabled' if enabled else 'disabled'}")
                    ...
                    if file_age_hours > max_age_hours:
                        ...
                        try:
                            ...
                            if heartbeat_age_hours > max_age_hours:
                                ...
                                self._stale_locks_cleaned[hook_name] += 1
                                self.logger.info(f"Cleaned stale lock: {lock_file} (age: {heartbeat_age_hours:.2f}h)")
    ...
    def get_global_lock_stats(self) -> dict[str, t.Any]:
        ...
        all_hooks = set(self._global_lock_attempts.keys()) | set(self._global_lock_successes.keys()) | set(self._global_lock_failures.keys())
    ...
    def configure_from_options(self, options: t.Any) -> None:
        ...
        self.logger.info(f"Configured lock manager: global_locks={'enabled' if self._global_lock_enabled else 'disabled'}, timeout={self._global_config.timeout_seconds}s, lock_dir={self._global_config.lock_directory}")
    ...
    def get_comprehensive_status(self) -> dict[str, t.Any]:
        ...
        status = {
            ...
            "currently_locked_hooks": [
                hook for hook in self._hooks_requiring_locks 
