...
class RuffJSONParser(JSONParser):
    ...
    def _parse_ruff_item(self, item: object) -> Issue | None:
        ...
        if not all((k in item for k in required_fields)):
            ...
            logger.warning(f'Skipping ruff item missing required fields {missing}: {item}')
            return None
        ...
        if line_number is None and 'location' in item:
            logger.warning(f"Invalid location format in ruff output: {item['location']}")
            return None
        code = str(item['code'])
        message = str(item['message'])
        return Issue(type=self._get_issue_type(code), severity=self._get_severity(code), message=f'{code} {message}', file_path=file_path, line_number=line_number, stage='ruff-check', details=self._build_ruff_details(item, code))
...
class MypyJSONParser(JSONParser):
    ...
    def _validate_mypy_item(self, item: dict) -> bool:
        ...
        if not all((k in item for k in required_fields)):
            missing = required_fields - item.keys()
            logger.warning(f'Skipping mypy item missing required fields {missing}: {item}')
            return False
        return True
    ...
    def _build_mypy_issue(self, item: dict) -> Issue:
        ...
        return Issue(type=IssueType.TYPE_ERROR, severity=severity, message=message, file_path=file_path, line_number=line_number, stage='mypy', details=[f'severity: {severity_str}'])
...
class BanditJSONParser(JSONParser):
    ...
    def parse_json(self, data: dict[str, object] | list[object]) -> list[Issue]:
        ...
        for item in results:
            try:
                if not isinstance(item, dict):
                    logger.warning(f'Skipping non-dict item in bandit output: {type(item)}')
                    continue
                required_fields = ['filename', 'issue_text', 'line_number']
                if not all((k in item for k in required_fields)):
                    missing = required_fields - item.keys()
                    logger.warning(f'Skipping bandit item missing required fields {missing}: {item}')
                    continue
                file_path = str(item['filename'])
                ...
                issues.append(Issue(type=IssueType.SECURITY, severity=severity, message=f'{test_id}: {message}', file_path=file_path, line_number=line_number, stage='bandit', details=[f'test_id: {test_id}', f'severity: {severity_str}']))
            except Exception as e:
    ...
    def _map_severity(self, severity_str: str) -> Priority:
        mapping = {'HIGH': Priority.CRITICAL, 'MEDIUM': Priority.HIGH, 'LOW': Priority.MEDIUM}
...
class ComplexipyJSONParser(JSONParser):
    ...
    def _find_json_path(self, output: str) -> str | None:
        ...
        patterns = ['complexipy_results_*.json', 'complexipy.json', '.complexipy_cache/*.json', '.complexipy/*.json']
        ...
        if output_dir.exists():
            matches = sorted(output_dir.glob('complexipy_results_*.json'), key=lambda p: p.stat().st_mtime, reverse=True)
            if matches:
                logger.debug(f'Found complexipy JSON in cache: {matches[0]}')
                return str(matches[0])
        return None

    def _extract_line_number_tier1(self, file_path: str, function_name: str) -> int | None:
        ...
        line_number = self._search_ast_for_line_number(file_path, function_name, search_names)
        return line_number
    ...
    def _search_ast_for_line_number(self, file_path: str, function_name: str, search_names: list[str]) -> int | None:
        import ast
        from pathlib import Path
        try:
            ...
            for search_name in search_names:
                if (line_number := self._find_function_in_ast(tree, search_name)):
                    ...
                    logger.debug(f"Found line number for '{function_name}' (searched as '{search_name}') in {file_path}: {line_number}")
                    return line_number
            logger.debug(f"Function '{function_name}' not found in {file_path} (searched for: {search_names})")
            return None
        except (SyntaxError, OSError, UnicodeDecodeError) as e:
            logger.debug(f"Failed to extract line number for '{function_name}' in {file_path}: {e}")
            return None
        except Exception as e:
            logger.warning(f"Unexpected error extracting line number for '{function_name}' in {file_path}: {e}")
            return None
    ...
    def parse_json(self, data: dict[str, object] | list[object]) -> list[Issue]:
        ...
        logger.info(f'Parsed {len(issues)} issues from complexipy JSON output (filtered from {len(data)} total functions, threshold: >{self.max_complexity})')
        return issues
    ...
    def _parse_complexipy_item(self, item: object) -> Issue | None:
        ...
        if not all((k in item for k in required_fields)):
            missing = required_fields - item.keys()
            logger.warning(f'Skipping complexipy item missing required fields {missing}: {item}')
            return None
    ...
    def _validate_complexity_value(self, complexity: object) -> bool:
        if not isinstance(complexity, int):
            logger.warning(f'Skipping invalid complexity value: {complexity} (type: {type(complexity)})')
            return False
        return True

    def _is_complexity_above_threshold(self, complexity: int) -> bool:
        if complexity <= self.max_complexity:
            logger.debug(f'Skipping function with complexity {complexity} <= threshold {self.max_complexity}')
            return False
        return True
    ...
    def _create_complexipy_issue(self, item: dict, complexity: int) -> Issue:
        ...
        return Issue(type=IssueType.COMPLEXITY, severity=severity, message=message, file_path=file_path, line_number=line_number, stage='complexity', details=details)
    ...
    def _build_complexipy_details(self, complexity: int, function_name: str, line_number: int | None) -> list[str]:
        details = [f'complexity: {complexity}', f'function: {function_name}', f'threshold: >{self.max_complexity}']
        if line_number:
    ...
    def get_issue_count(self, data: dict[str, object] | list[object]) -> int:
        if isinstance(data, list):
            return sum((1 for item in data if isinstance(item, dict) and isinstance(item.get('complexity'), int) and (item['complexity'] > self.max_complexity)))
        return 0
    ...
    def _find_class_method_in_ast(self, tree: t.Any, search_name: str) -> int | None:
        ...
        if (line_number := self._search_method_in_class(tree, class_name, method_name)):
            return line_number
        logger.debug(f"Could not find class-qualified method '{search_name}', falling back to bare name '{method_name}'")
        return self._find_simple_function_in_ast(tree, method_name)

    def _search_method_in_class(self, tree: t.Any, class_name: str, method_name: str) -> int | None:
    ...
    def _find_method_in_class_node(self, class_node: t.Any, method_name: str) -> int | None:
        import ast
    ...
    def _find_simple_function_in_ast(self, tree: t.Any, function_name: str) -> int | None:
        import ast
        for node in ast.walk(t.cast(ast.AST, tree)):
    ...
    def _is_function_def_with_name(self, node: t.Any, function_name: str) -> bool:
        import ast
        return isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)) and node.name == function_name
...
class SemgrepJSONParser(JSONParser):
    ...
    def _parse_semgrep_item(self, item: object) -> Issue | None:
        ...
        return Issue(type=IssueType.SECURITY, severity=self._map_severity(severity_str), message=f'{check_id}: {message}', file_path=path, line_number=line_number, stage='semgrep', details=[f'check_id: {check_id}', f'severity: {severity_str}'])
    ...
    def _map_severity(self, severity_str: str) -> Priority:
        mapping = {'ERROR': Priority.CRITICAL, 'WARNING': Priority.HIGH, 'INFO': Priority.MEDIUM}
        return mapping.get(severity_str.upper(), Priority.MEDIUM)
...
class PipAuditJSONParser(JSONParser):
    ...
    def _parse_dependency(self, dep: object) -> list[Issue]:
        if not isinstance(dep, dict):
            logger.warning(f'Skipping non-dict item in pip-audit dependencies: {type(dep)}')
            return []
        ...
        if not isinstance(vulns, list):
            ...
            return []
        return [self._create_vulnerability_issue(name, vuln) for vuln in vulns if isinstance(vuln, dict)]
    ...
    def _create_vulnerability_issue(self, package_name: str, vuln: dict) -> Issue:
        ...
        return Issue(type=IssueType.SECURITY, severity=self._map_severity(severity_str), message=f'{vuln_id}: {description}', file_path=None, line_number=None, stage='pip-audit', details=[f'package: {package_name}', f'vulnerability_id: {vuln_id}', f'severity: {severity_str}'])
    ...
    def _get_dependencies_list(self, data: dict[str, object] | list[object]) -> list[object] | None:
...
class GitleaksJSONParser(JSONParser):
    ...
    def _extract_json_from_output(self, output: str) -> dict[str, object] | list[object] | None:
        import re
        if not output.strip():
            return None
    ...
    def parse_json(self, data: dict[str, object] | list[object]) -> list[Issue]:
        ...
        for item in data:
            try:
                if not isinstance(item, dict):
                    logger.warning(f'Skipping non-dict item in gitleaks output: {type(item)}')
                    continue
                ...
                issues.append(Issue(type=IssueType.SECURITY, severity=severity, message=message, file_path=file_path, line_number=line_number, stage='gitleaks', details=[f'rule_id: {rule_id}', f'severity: {severity_str}', f'secret_type: {description}']))
            except Exception as e:
    ...
    def get_issue_count(self, data: dict[str, object] | list[object]) -> int:
        if isinstance(data, dict) and 'tests' in data:
            ...
            if isinstance(tests, list):
                failed = [t for t in tests if isinstance(t, dict) and t.get('outcome') == 'failed']
                return len(failed)
        return 0
...
def register_json_parsers(factory: ParserFactory) -> None:
    ...
    logger.info('Registered JSON parsers: ruff, mypy, bandit, complexipy, semgrep, pip-audit, gitleaks, pytest')