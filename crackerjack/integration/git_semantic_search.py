...
class GitSemanticSearch:
        ...
        if self._git_collector is None:
            ...
            if self._git_collector_factory:
                ...
                self._git_collector = GitMetricsCollector(
                    repo_path=self.repo_path, executor=executor
                )
        ...
        if self._akosha_integration is None:
            ...
            if self._akosha_integration_factory:
                ...
                self._akosha_integration = create_akosha_git_integration(
                    repo_path=self.repo_path, backend="auto"
                )
        return self._akosha_integration

    async def search_git_history(
        self, query: str, limit: int = 10, days_back: int = 30
    ) -> dict[str, t.Any]:
        try:
    ...
    async def find_workflow_patterns(
        self, pattern_description: str, days_back: int = 90, min_frequency: int = 3
    ) -> dict[str, t.Any]:
        try:
            await self._ensure_index(days_back)
            akosha = self._get_akosha_integration()
            events = await akosha.search_git_history(
                query=pattern_description, limit=100
            )
            if not events:
                return {
                    "success": True,
    ...
    async def recommend_git_practices(
        self, focus_area: str = "general", days_back: int = 60
    ) -> dict[str, t.Any]:
        try:
            collector = self._get_git_collector()
            dashboard = collector.get_velocity_dashboard(days_back=days_back)
            recommendations = self._generate_recommendations(
                dashboard=dashboard, focus_area=focus_area, days_back=days_back
            )
            ...
            formatted_recommendations.sort(
    ...
    def _analyze_patterns(
        self, events: list[t.Any], pattern_description: str, min_frequency: int
    ...
    def _create_pattern_from_cluster(
        self, group_id: str, events: list[t.Any], pattern_description: str
    ...
    def _generate_recommendations(
        self, dashboard: t.Any, focus_area: str, days_back: int
...
def create_git_semantic_search(
    repo_path: str, config: GitSemanticSearchConfig | None = None
) -> GitSemanticSearch:
