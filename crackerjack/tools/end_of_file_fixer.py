...
def fix_end_of_file(file_path: Path) -> bool:
    try:
        ...
        if needs_fix and fixed_content is not None:
            ...
            print(f"Fixed end-of-file: {file_path}")
            return True
...
def _process_files_in_check_mode(files: list[Path]) -> int:
    ...
    for file_path in files:
        try:
            ...
            if needs_fix:
                print(f"Missing/incorrect end-of-file: {file_path}")
                modified_count += 1
        except Exception as e:
            print(f"Error processing {file_path}: {e}", file=sys.stderr)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if modified_count > 0:
        if args.check:
            print(f"\n{modified_count} file(s) with incorrect end-of-file")
        else:
            print(f"\nFixed {modified_count} file(s)")

            print("files were modified by this hook")
        return 1

    print("All files end with correct newline")
    return 0
