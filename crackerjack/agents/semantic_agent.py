...
class SemanticAgent(SubAgent):
    ...
    async def analyze_and_fix(self, issue: Issue) -> FixResult:
        ...
        try:
            ...
            result = await self._perform_semantic_analysis(file_path, vector_store, issue)
                            ...
                            "related_code": [
                                {
                                    ...
                                    "content": result.content[:200],  # Truncate for readability
        ...
        try:
            ...
            class CodeElementExtractor(ast.NodeVisitor):
                ...
                def visit_FunctionDef(self, node: ast.FunctionDef) -> None:
                    ...
                    if node.body and isinstance(node.body[0], ast.Expr):
                        ...
                        if hasattr(value, 's'):  # Old ast.Str format
                            docstring = str(value.s)[:100]
                        elif hasattr(value, 'value') and isinstance(value.value, str):  # New ast.Constant format
                            docstring = str(value.value)[:100]
                ...
                def visit_ClassDef(self, node: ast.ClassDef) -> None:
                    ...
                    if node.bases:
                        bases = [self._get_name(base) for base in node.bases[:2]]
            ...
            elements = extractor.elements[:10]  # Limit to top 10 elements

        except SyntaxError:
            # If parsing fails, extract using simple text patterns
            lines = content.split('\n')
    ...
    def _generate_semantic_recommendations(
        ...
        if related_patterns:
            ...
            high_similarity_count = sum(
                1 for pattern in related_patterns
    ...
    def _update_pattern_stats(self, result: FixResult) -> None:
        ...
        if result.success:
            self.pattern_stats["patterns_discovered"] += len(
                result.fixes_applied
            )
            self.pattern_stats["semantic_suggestions"] += len(
