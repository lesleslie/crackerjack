...
class QwenCodeBridge:
    def __init__(self, context: AgentContext) -> None:
        ...
        self._ai_available = _qwen_ai_available

        if not self._ai_available:
            self.logger.warning(
                "Qwen AI adapter not available - AI-powered fixes disabled",
            )
    ...
    def should_consult_external_agent(
        ...
        return issue.type in QWEN_CODE_AGENT_MAPPING

    def _get_agent_mapping(self) -> dict[t.Any, list[str]]:
        return QWEN_CODE_AGENT_MAPPING
    ...
    def get_recommended_external_agents(self, issue: Issue) -> list[str]:
        return QWEN_CODE_AGENT_MAPPING.get(issue.type, [])
    ...
    async def _ensure_ai_fixer(self) -> t.Any:
        if not self._ai_available:
            msg = "Qwen AI adapter not available - install optional AI dependencies"
            raise RuntimeError(
                msg,
            )
    ...
    async def consult_on_issue(
        ...
        try:
            ...
            self.logger.info(
                f"Consulting Qwen AI for {fix_type} issue in {file_path}",
            )
