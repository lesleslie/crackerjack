...
class ArchitectAgent(ProactiveAgent):
    ...
    def get_supported_types(self) -> set[IssueType]:
        return {IssueType.TYPE_ERROR, IssueType.TEST_ORGANIZATION}
    ...
    async def _get_specialist_plan(self, issue: Issue) -> dict[str, t.Any]:
        return {'strategy': 'external_specialist_guided', 'specialist': 'crackerjack-architect', 'approach': self._get_specialist_approach(issue), 'patterns': self._get_recommended_patterns(issue), 'dependencies': self._analyze_dependencies(issue), 'risks': self._identify_risks(issue), 'validation': self._get_validation_steps(issue)}

    async def _get_internal_plan(self, issue: Issue) -> dict[str, t.Any]:
        return {'strategy': 'internal_pattern_based', 'approach': self._get_internal_approach(issue), 'patterns': self._get_cached_patterns_for_issue(issue), 'dependencies': [], 'risks': ['minimal'], 'validation': ['run_quality_checks']}
    ...
    def _get_internal_approach(self, issue: Issue) -> str:
        return {IssueType.FORMATTING: 'apply_standard_formatting', IssueType.IMPORT_ERROR: 'optimize_imports', IssueType.TYPE_ERROR: 'add_type_annotations', IssueType.TEST_FAILURE: 'fix_test_patterns', IssueType.DEAD_CODE: 'remove_unused_code', IssueType.DOCUMENTATION: 'update_documentation'}.get(issue.type, 'apply_standard_fix')

    def _get_recommended_patterns(self, issue: Issue) -> list[str]:
        return {IssueType.COMPLEXITY: ['extract_method', 'dependency_injection', 'protocol_interfaces', 'helper_methods'], IssueType.DRY_VIOLATION: ['common_base_class', 'utility_functions', 'protocol_pattern', 'composition'], IssueType.PERFORMANCE: ['list_comprehension', 'generator_pattern', 'caching', 'algorithm_optimization'], IssueType.SECURITY: ['secure_temp_files', 'input_validation', 'safe_subprocess', 'token_handling']}.get(issue.type, ['standard_patterns'])
    ...
    def _get_cached_patterns_for_issue(self, issue: Issue) -> list[str]:
        ...
        for pattern_key, pattern_data in cached.items():
            if pattern_key.startswith(issue.type.value):
                matching_patterns.extend(pattern_data.get('plan', {}).get('patterns', []))
        return matching_patterns or ['default_pattern']

    def _analyze_dependencies(self, issue: Issue) -> list[str]:
        dependencies = []
        if issue.type == IssueType.COMPLEXITY:
            dependencies.extend(['update_tests_for_extracted_methods', 'update_type_annotations', 'verify_imports'])
        if issue.type == IssueType.DRY_VIOLATION:
    ...
    def _identify_risks(self, issue: Issue) -> list[str]:
        risks = []
        if issue.type == IssueType.COMPLEXITY:
            risks.extend(['breaking_existing_functionality', 'changing_method_signatures', 'test_failures'])
        if issue.type == IssueType.DRY_VIOLATION:
            risks.extend(['breaking_dependent_code', 'performance_impact'])
        return risks

    def _get_validation_steps(self, issue: Issue) -> list[str]:
        return ['run_fast_hooks', 'run_full_tests', 'run_comprehensive_hooks', 'validate_complexity_reduction', 'check_pattern_compliance']

    async def execute_with_plan(self, issue: Issue, plan: dict[str, t.Any]) -> FixResult:
        strategy = plan.get('strategy', 'internal_pattern_based')
        if strategy == 'external_specialist_guided':
            self.log(f'Warning: execute_with_plan() called for specialist issue {issue.type.value}')
            return FixResult(success=False, confidence=0.0, remaining_issues=[f'Issue type {issue.type.value} should be delegated to specialist'])
        if issue.type == IssueType.TYPE_ERROR:
        ...
        return FixResult(success=False, confidence=0.0, remaining_issues=[f'ArchitectAgent does not handle {issue.type.value} proactively'])
    ...
    async def _fix_type_error_with_plan(self, issue: Issue, plan: dict[str, t.Any]) -> FixResult:
        ...
        if not issue.file_path:
            return FixResult(success=False, confidence=0.0, remaining_issues=[f'Cannot fix type error without file path: {issue.message}'], recommendations=['Provide file path for type error fixing'])
        ...
        return FixResult(success=True, confidence=confidence, fixes_applied=fixes_applied, remaining_issues=[], recommendations=[f'Fixed type error: {issue.message}'], files_modified=[issue.file_path])

    def _apply_type_error_fixes(self, content: str, error_message: str) -> tuple[str, list[str]]:
        fixes_applied = []
        fixed_content = content
        fixed_content, new_fixes = self._fix_missing_typing_imports(fixed_content, error_message, content)
        fixes_applied.extend(new_fixes)
        fixed_content, new_fixes = self._fix_any_builtin_type(fixed_content, error_message)
        fixes_applied.extend(new_fixes)
        fixed_content, new_fixes = self._fix_missing_annotations_type(fixed_content, error_message)
        fixes_applied.extend(new_fixes)
        fixed_content, new_fixes = self._fix_path_str_type_conversion(fixed_content, error_message)
        fixes_applied.extend(new_fixes)
        fixed_content, new_fixes = self._fix_missing_await(fixed_content, error_message)
        fixes_applied.extend(new_fixes)
        return (fixed_content, fixes_applied)

    def _fix_missing_typing_imports(self, content: str, error_message: str, original_content: str) -> tuple[str, list[str]]:
        imports_needed = []
    ...
    def _fix_any_builtin_type(self, content: str, error_message: str) -> tuple[str, list[str]]:
    ...
    def _fix_missing_annotations_type(self, content: str, error_message: str) -> tuple[str, list[str]]:
    ...
    def _fix_path_str_type_conversion(self, content: str, error_message: str) -> tuple[str, list[str]]:
    ...
    def _fix_missing_await(self, content: str, error_message: str) -> tuple[str, list[str]]:
    ...
    def _add_typing_imports(self, content: str, imports_needed: list[str]) -> str:
        ...
        typing_import_idx, existing_typing_imports = self._find_existing_typing_imports(lines, imports_needed)
        ...
        if typing_import_idx is not None:
            lines = self._merge_existing_imports(lines, typing_import_idx, typing_imports_to_add)
        else:
            insert_idx = self._find_import_insertion_point(lines)
            lines.insert(insert_idx, import_line)
        return ''.join(lines)

    def _find_existing_typing_imports(self, lines: list[str], imports_needed: list[str]) -> tuple[int | None, set[str]]:
    ...
    def _merge_existing_imports(self, lines: list[str], typing_import_idx: int, imports_to_add: set[str]) -> list[str]:
    ...
    async def _fix_dependency_with_plan(self, issue: Issue, plan: dict[str, t.Any]) -> FixResult:
        self.log(f'Dependency fixing not yet implemented: {issue.message}')
        return FixResult(success=False, confidence=0.0, remaining_issues=[f'Dependency issue: {issue.message}'])

    async def _fix_documentation_with_plan(self, issue: Issue, plan: dict[str, t.Any]) -> FixResult:
        self.log(f'Documentation fixing not yet implemented: {issue.message}')
        return FixResult(success=False, confidence=0.0, remaining_issues=[f'Documentation issue: {issue.message}'])

    async def _fix_test_organization_with_plan(self, issue: Issue, plan: dict[str, t.Any]) -> FixResult:
        self.log(f'Test organization fixing not yet implemented: {issue.message}')
        return FixResult(success=False, confidence=0.0, remaining_issues=[f'Test organization issue: {issue.message}'])
    ...
    async def execute_fix_plan(self, plan: 'FixPlan') -> 'FixResult':
        self.log(f'Executing FixPlan for {plan.file_path}:{plan.issue_type} ({len(plan.changes)} changes, risk={plan.risk_level})')
        if not plan.changes:
            self.log(f'Plan has no changes to apply for {plan.file_path}', level='WARNING')
            return FixResult(success=False, confidence=0.0, remaining_issues=['Plan has no changes to apply'], recommendations=['PlanningAgent should generate actual changes'])
        if plan.issue_type == 'TYPE_ERROR':
            issue = Issue(type=IssueType.TYPE_ERROR, severity=plan.changes[0].line_range[0] > 30 and Priority.HIGH or Priority.MEDIUM, message=plan.rationale, file_path=plan.file_path)
            plan_dict = {'strategy': 'internal_pattern_based', 'approach': 'add_type_annotations'}
    ...
    async def _apply_plan_changes(self, plan: 'FixPlan') -> 'FixResult':
        if not plan.file_path:
            return FixResult(success=False, confidence=0.0, remaining_issues=['No file path in plan'])
        try:
            file_content = await self._read_file_context(plan.file_path)
        except Exception as e:
            return FixResult(success=False, confidence=0.0, remaining_issues=[f'Could not read file: {e}'])
        applied_changes = []
        ...
        return FixResult(success=success, confidence=0.7 if success else 0.0, fixes_applied=applied_changes, remaining_issues=[] if success else ['Some changes failed'], files_modified=[plan.file_path] if success else [])
