...
def benchmark_service(mock_filesystem, mock_console):
    ...
    with patch('crackerjack.services.performance_benchmarks.Path.cwd') as mock_cwd:
        ...
        with patch('crackerjack.services.performance_benchmarks.Path.mkdir') as mock_mkdir:
    ...
    def test_service_initialization(self, mock_filesystem, mock_console):
        ...
        with patch('crackerjack.services.performance_benchmarks.Path.cwd') as mock_cwd:
            ...
            with patch('crackerjack.services.performance_benchmarks.Path.mkdir') as mock_mkdir:
                ...
                assert service.history_file == Path("/test/project/.benchmarks/performance_history.json")
...
class TestComprehensiveBenchmark:
    ...
    def test_run_comprehensive_benchmark_full(self, mock_time, benchmark_service):
        ...
        with patch.object(benchmark_service, '_benchmark_test_suite') as mock_tests, \
             patch.object(benchmark_service, '_benchmark_hooks') as mock_hooks, \
             patch.object(benchmark_service, '_benchmark_workflow_components') as mock_components, \
             patch.object(benchmark_service, '_benchmark_file_operations') as mock_file_ops, \
             patch.object(benchmark_service, '_generate_performance_recommendations') as mock_recommendations, \
            ...
            # Configure mocks
            mock_tests.return_value = {"iter_1": {"success": True, "total_duration": 5.0}}
            ...
            # Verify execution
            assert report.total_duration == 10.5
            assert report.test_benchmarks == {"iter_1": {"success": True, "total_duration": 5.0}}
...
class TestTestSuiteBenchmarking:
    ...
    def test_benchmark_test_suite_success(self, mock_path_open, mock_path_exists, mock_time, mock_subprocess, benchmark_service):
        ...
        benchmark_data = {
            "benchmarks": [
                {"name": "test_func", "mean": 0.001, "stddev": 0.0001}
            ]
        }
        ...
        mock_subprocess.assert_has_calls([
            call(expected_cmd, check=False, capture_output=True, text=True, timeout=300),
            call(expected_cmd, check=False, capture_output=True, text=True, timeout=300)
        ])
    ...
    @patch('crackerjack.services.performance_benchmarks.time.time')
    @patch('pathlib.Path.exists')
    def test_benchmark_test_suite_no_benchmark_file(self, mock_path_exists, mock_time, mock_subprocess, benchmark_service):
...
class TestHookBenchmarking:
    ...
    @patch('crackerjack.services.performance_benchmarks.subprocess.run')
    @patch('crackerjack.services.performance_benchmarks.time.time')
    def test_benchmark_hooks_success(self, mock_time, mock_subprocess, benchmark_service):
        """Test successful hook benchmarking."""
        # Mock time progression for multiple hooks and iterations
        mock_time.side_effect = [
            1000.0, 1002.0,  # First hook, first iteration
            1005.0, 1007.0,  # First hook, second iteration  
            1010.0, 1015.0,  # Second hook, first iteration
            1020.0, 1022.0,  # Second hook, second iteration
        ]
        ...
        with patch.object(benchmark_service, '_benchmark_hooks') as mock_method:
            ...
            result = benchmark_service._benchmark_hooks(iterations=2)
            
            mock_method.assert_called_once_with(2)

    @patch('crackerjack.services.performance_benchmarks.subprocess.run')
    @patch('crackerjack.services.performance_benchmarks.time.time')
    def test_benchmark_hooks_individual_success(self, mock_time, mock_subprocess, benchmark_service):
        ...
        original_method = benchmark_service._benchmark_hooks
    ...
    @patch('crackerjack.services.performance_benchmarks.subprocess.run')
    @patch('crackerjack.services.performance_benchmarks.time.time')
    def test_benchmark_hooks_timeout_handling(self, mock_time, mock_subprocess, benchmark_service):
...
class TestWorkflowComponentBenchmarking:
    ...
    @patch('pathlib.Path.open')
    @patch('tomllib.load')
    def test_benchmark_workflow_components_file_discovery(self, mock_tomllib, mock_path_open, mock_path_exists, mock_rglob, mock_time, benchmark_service):
        """Test file discovery component benchmarking."""
        mock_time.side_effect = [1000.0, 1001.5, 1002.0, 1002.2]  # File discovery and config loading
        
        # Mock file system operations
        python_files = [Path("test1.py"), Path("test2.py"), Path("test3.py")]
        mock_rglob.return_value = python_files
        mock_path_exists.return_value = True
        mock_path_open.return_value = mock_open(read_data=b'[tool.pytest.ini_options]\ntestpaths = ["tests"]')()
    ...
    @patch('pathlib.Path.rglob')
    @patch('pathlib.Path.exists')
    def test_benchmark_workflow_components_no_config(self, mock_path_exists, mock_rglob, mock_time, benchmark_service):
    ...
    @patch('pathlib.Path.open')
    @patch('tomllib.load')
    def test_benchmark_workflow_components_config_load_error(self, mock_tomllib, mock_path_open, mock_path_exists, mock_rglob, mock_time, benchmark_service):
...
class TestFileOperationsBenchmarking:
    ...
    def test_benchmark_file_operations_success(self, mock_read_text, mock_glob, mock_time, benchmark_service):
    ...
    @patch('pathlib.Path.glob')
    @patch('pathlib.Path.read_text')
    def test_benchmark_file_operations_read_error(self, mock_read_text, mock_glob, mock_time, benchmark_service):
...
class TestPerformanceRecommendations:
    ...
    def test_generate_performance_recommendations_comprehensive(self, benchmark_service):
        ...
        report = PerformanceReport(
            ...
            hook_performance={
                "pyright": {"mean_duration": 45.0, "min_duration": 40.0, "max_duration": 50.0},  # Slow hook
                "ruff": {"mean_duration": 2.0, "min_duration": 1.8, "max_duration": 2.2}  # Fast hook
            },
            workflow_benchmarks=[
        ...
        recommendations = benchmark_service._generate_performance_recommendations(report)
        ...
        hook_rec = next((r for r in recommendations if "Slow hooks detected" in r), None)
        assert hook_rec is not None
        assert "pyright(45.0s)" in hook_rec
        
        component_rec = next((r for r in recommendations if "Slow workflow components" in r), None)
        assert component_rec is not None
        assert "slow_component" in component_rec
        
        overall_rec = next((r for r in recommendations if "Overall workflow execution is slow" in r), None)
    ...
    def test_generate_performance_recommendations_no_issues(self, benchmark_service):
        ...
        report = PerformanceReport(
            ...
            test_benchmarks={"iteration_1": {"total_duration": 15.0, "success": True}},  # Fast test
    ...
    def test_identify_slow_hooks(self, benchmark_service):
        ...
        hook_performance = {
            "fast_hook": {"mean_duration": 5.0, "min_duration": 4.0, "max_duration": 6.0},
            "slow_hook": {"mean_duration": 35.0, "min_duration": 30.0, "max_duration": 40.0},
            "very_slow_hook": {"mean_duration": 50.0, "min_duration": 45.0, "max_duration": 55.0},
    ...
    def test_identify_slow_components(self, benchmark_service):
        ...
        slow_components = benchmark_service._identify_slow_components(workflow_benchmarks)
...
class TestHistoryManagement:
    ...
    @patch('json.dump')
    @patch('crackerjack.services.performance_benchmarks.time.time')
    def test_save_performance_history_new_file(self, mock_time, mock_json_dump, mock_open_builtin, mock_path_exists, benchmark_service):
    ...
    @patch('json.dump')
    @patch('crackerjack.services.performance_benchmarks.time.time')
    def test_save_performance_history_existing_file(self, mock_time, mock_json_dump, mock_json_load, mock_open_builtin, mock_path_exists, benchmark_service, sample_performance_history):
    ...
    @patch('json.dump')
    @patch('crackerjack.services.performance_benchmarks.time.time')
    def test_save_performance_history_limit_records(self, mock_time, mock_json_dump, mock_json_load, mock_open_builtin, mock_path_exists, benchmark_service):
        """Test performance history record limiting (keep last 50)."""
        # Create history with 51 records
        large_history = [
            {"timestamp": i, "total_duration": 10.0}
            for i in range(51)
        ]
    ...
    @patch('pathlib.Path.exists')
    def test_save_performance_history_error_handling(self, mock_path_exists, benchmark_service):
    ...
    @patch('builtins.open')
    @patch('json.load')
    def test_load_performance_history_success(self, mock_json_load, mock_open_builtin, mock_path_exists, benchmark_service, sample_performance_history):
    ...
    @patch('pathlib.Path.exists')
    def test_load_performance_history_no_file(self, mock_path_exists, benchmark_service):
    ...
    @patch('builtins.open')
    @patch('json.load')
    def test_load_performance_history_insufficient_data(self, mock_json_load, mock_open_builtin, mock_path_exists, benchmark_service):
        """Test performance history loading with insufficient data."""
        insufficient_history = [{"timestamp": 1000.0, "total_duration": 10.0}]  # Only 1 record
...
class TestBaselineComparison:
    ...
    def test_compare_with_baseline_success(self, benchmark_service, sample_performance_history):
        ...
        with patch.object(benchmark_service, '_load_performance_history', return_value=sample_performance_history):
            ...
            assert abs(comparison["overall_performance_change_percent"] - expected_change) < 0.1
    ...
    def test_compare_with_baseline_no_history(self, benchmark_service):
        ...
        with patch.object(benchmark_service, '_load_performance_history', return_value=None):
            comparison = benchmark_service._compare_with_baseline(current_report)
            assert comparison == {}
    ...
    def test_compare_with_baseline_error(self, benchmark_service):
        ...
        with patch.object(benchmark_service, '_load_performance_history', side_effect=Exception("Load error")):
            comparison = benchmark_service._compare_with_baseline(current_report)
...
class TestTrendAnalysis:
    ...
    def test_get_performance_trends_success(self, benchmark_service, sample_performance_history):
        ...
        with patch.object(benchmark_service, '_get_recent_history', return_value=extended_history):
    ...
    def test_get_performance_trends_insufficient_data(self, benchmark_service):
        ...
        insufficient_history = [{"timestamp": time.time(), "total_duration": 10.0}]
    ...
    def test_get_performance_trends_exception(self, benchmark_service):
        ...
        with patch.object(benchmark_service, '_get_recent_history', side_effect=Exception("Analysis error")):
            trends = benchmark_service.get_performance_trends(days=7)
    ...
    @patch('json.load')
    @patch('crackerjack.services.performance_benchmarks.time.time')
    def test_get_recent_history_filtering(self, mock_time, mock_json_load, mock_open_builtin, mock_path_exists, benchmark_service):
        """Test recent history filtering by time period."""
        current_time = time.time()
        
        # Create mixed history with old and recent entries
        mixed_history = [
            {"timestamp": current_time - (10 * 86400), "total_duration": 50.0},  # 10 days ago (old)
            {"timestamp": current_time - (5 * 86400), "total_duration": 30.0},   # 5 days ago (recent)
            {"timestamp": current_time - (2 * 86400), "total_duration": 25.0},   # 2 days ago (recent)
            {"timestamp": current_time - (1 * 86400), "total_duration": 20.0},   # 1 day ago (recent)
        ]
    ...
    def test_extract_component_durations(self, benchmark_service):
        ...
        durations = benchmark_service._extract_component_durations(history, "file_discovery")
        assert durations == [1.0, 1.2, 0.9]
        
        # Extract config_loading durations (missing from some entries)
        durations = benchmark_service._extract_component_durations(history, "config_loading")
        assert durations == [0.5, 0.6]  # Should skip entries without the component
        
        # Extract non-existent component
        durations = benchmark_service._extract_component_durations(history, "non_existent")
        assert durations == []
...
class TestDisplayFunctionality:
    ...
    def test_display_performance_report_full(self, benchmark_service):
        ...
        report = PerformanceReport(
            ...
            hook_performance={
                "ruff": {"mean_duration": 3.2, "min_duration": 2.8, "max_duration": 3.6},
                "pyright": {"mean_duration": 12.5, "min_duration": 11.0, "max_duration": 14.0}
            },
            baseline_comparison={
        ...
        assert len(console_calls) >= 5  # Header, stats, tables, comparison, recommendations, footer
    ...
    def test_display_performance_report_minimal(self, benchmark_service):
        ...
        overall_stats_call = next((call for call in console_calls 
                                 if "Total Duration: 15.00s" in str(call)), None)
        assert overall_stats_call is not None
        
        benchmarks_dir_call = next((call for call in console_calls 
                                  if "Benchmark data saved to" in str(call)), None)
        assert benchmarks_dir_call is not None
        
        # Should show "No performance issues detected!" for empty recommendations
        no_issues_call = next((call for call in console_calls 
                             if "No performance issues detected!" in str(call)), None)
    ...
    def test_print_comparison_metrics(self, benchmark_service):
        ...
        improvement_call = next((call for call in console_calls 
                               if "12.5% faster" in str(call) and "[green]" in str(call)), None)
        assert improvement_call is not None
        
        minor_degradation_call = next((call for call in console_calls 
                                     if "8.3% slower" in str(call) and "[yellow]" in str(call)), None)
        assert minor_degradation_call is not None
        
        major_degradation_call = next((call for call in console_calls 
                                     if "25.7% slower" in str(call) and "[red]" in str(call)), None)
...
class TestPerformanceBenchmarkingIntegration:
    ...
    def test_comprehensive_benchmark_integration(self, mock_filesystem, mock_console):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('crackerjack.services.performance_benchmarks.Path.cwd', return_value=temp_path):
                service = PerformanceBenchmarkService(
                    filesystem=mock_filesystem,
                    console=mock_console
                )
                
                with patch.object(service, '_benchmark_test_suite') as mock_tests, \
                     patch.object(service, '_benchmark_hooks') as mock_hooks, \
                     patch.object(service, '_benchmark_workflow_components') as mock_components, \
                     patch.object(service, '_benchmark_file_operations') as mock_file_ops, \
    ...
    @patch('json.load')
    @patch('crackerjack.services.performance_benchmarks.time.time')
    def test_trend_analysis_integration(self, mock_time, mock_json_load, mock_open_builtin, mock_path_exists, benchmark_service):
