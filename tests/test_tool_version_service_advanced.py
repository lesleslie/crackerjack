...
class TestToolVersionServiceAdvanced:
    ...
    def test_service_initialization(self, service: ToolVersionService, console: Console) -> None:
    ...
    def test_get_ruff_version_success(self, mock_run: Mock, service: ToolVersionService) -> None:
    ...
    @patch('subprocess.run')
    def test_get_ruff_version_failure(self, mock_run: Mock, service: ToolVersionService) -> None:
    ...
    @patch('subprocess.run')
    def test_get_pyright_version_success(self, mock_run: Mock, service: ToolVersionService) -> None:
    ...
    @patch('subprocess.run')
    def test_get_pyright_version_npm_fallback(self, mock_run: Mock, service: ToolVersionService) -> None:
    ...
    @patch('subprocess.run')
    def test_get_precommit_version_success(self, mock_run: Mock, service: ToolVersionService) -> None:
    ...
    @patch('subprocess.run')
    def test_get_uv_version_success(self, mock_run: Mock, service: ToolVersionService) -> None:
        """Test successful uv version detection."""
        mock_run.return_value = Mock(
            stdout="uv 0.2.18 (Cargo 1.75.0)",
            stderr="",
            returncode=0
        )
    ...
    @patch('subprocess.run')
    def test_check_installed_tools_comprehensive(self, mock_run: Mock, service: ToolVersionService) -> None:
    ...
    @patch('subprocess.run')
    def test_check_installed_tools_with_failures(self, mock_run: Mock, service: ToolVersionService) -> None:
    ...
    def test_create_not_installed_version_info(self, service: ToolVersionService) -> None:
    ...
    @pytest.mark.asyncio
    async def test_fetch_latest_version_success(self, service: ToolVersionService) -> None:
    ...
    @pytest.mark.asyncio
    async def test_fetch_latest_version_failure(self, service: ToolVersionService) -> None:
    ...
    @pytest.mark.asyncio
    async def test_check_tool_updates_comprehensive(self, service: ToolVersionService) -> None:
        ...
        with patch.object(service, 'check_installed_tools') as mock_installed:
            with patch.object(service, '_fetch_latest_version') as mock_fetch:
                ...
                mock_fetch.side_effect = ["0.1.6", "1.1.365"]  # ruff has update, pyright current
    ...
    async def test_concurrent_version_checks(self, service: ToolVersionService) -> None:
        ...
        with patch.object(service, '_fetch_latest_version') as mock_fetch:
            ...
            assert results == ["ruff-1.0.0", "pyright-1.0.0", "uv-1.0.0", "pre-commit-1.0.0"]
...
class TestToolVersionServiceIntegration:
    ...
    def test_real_version_detection_integration(self, service: ToolVersionService) -> None:
        ...
        try:
            ...
            detected_tools = [info for info in results if info.current_version != "not installed"]
