"""Strategic test coverage for core/autofix_coordinator.py - Auto-fixing coordination."""
...
class TestAutofixCoordinator:
    """Test autofix coordination functionality."""
    ...
    def test_apply_autofix_for_hooks_comprehensive_mode(self, coordinator):
        ...
        with patch.object(coordinator, '_should_skip_autofix', return_value=False), \
             patch.object(coordinator, '_apply_comprehensive_stage_fixes', return_value=True):
    ...
    def test_public_interface_methods(self, coordinator):
        ...
        with patch.object(coordinator, '_apply_fast_stage_fixes', return_value=True) as mock_fast, \
             patch.object(coordinator, '_apply_comprehensive_stage_fixes', return_value=True) as mock_comp, \
             patch.object(coordinator, '_run_fix_command', return_value=True) as mock_run, \
             patch.object(coordinator, '_check_tool_success_patterns', return_value=True) as mock_check, \
             patch.object(coordinator, '_validate_fix_command', return_value=True) as mock_validate_cmd, \
             patch.object(coordinator, '_validate_hook_result', return_value=True) as mock_validate_hook, \
             patch.object(coordinator, '_should_skip_autofix', return_value=False) as mock_skip:
    ...
    def test_execute_fast_fixes(self, coordinator):
        ...
        with patch.object(coordinator, '_run_fix_command', return_value=True) as mock_run:
    ...
    def test_is_successful_fix(self, coordinator):
        ...
        # Test "reformatted" pattern
        result = Mock(stdout="Reformatted code")
        assert coordinator._is_successful_fix(result) is True
        
        # Test no success pattern
    ...
    def test_check_tool_success_patterns(self, coordinator):
        ...
        # Test subprocess result
        cmd = ["uv", "run", "ruff"]
        result = Mock(returncode=0)
        assert coordinator._check_tool_success_patterns(cmd, result) is True
        
        # Test string result with ruff
        cmd = ["uv", "run", "ruff"]
        assert coordinator._check_tool_success_patterns(cmd, "Fixed 5 files") is True
        assert coordinator._check_tool_success_patterns(cmd, "would reformat 3 files") is True
        
        # Test trailing-whitespace
        cmd = ["uv", "run", "trailing-whitespace"]
        assert coordinator._check_tool_success_patterns(cmd, "Fixing whitespace") is True

    def test_validate_fix_command(self, coordinator):
        """Test fix command validation."""
        # Valid commands
        assert coordinator._validate_fix_command(["uv", "run", "ruff", "format"]) is True
        assert coordinator._validate_fix_command(["uv", "run", "bandit", "-ll"]) is True
    ...
    def test_validate_hook_result(self, coordinator):
        ...
        # Invalid result - no name attribute
        invalid_result1 = object()  # Basic object with no attributes
        assert coordinator._validate_hook_result(invalid_result1) is False
        
        # Invalid result - empty name
    ...
    def test_should_skip_autofix(self, coordinator):
        ...
        # ModuleNotFoundError
        import_error_result = Mock(raw_output="ModuleNotFoundError: No module named 'test'")
...
class TestAutofixCoordinatorIntegration:
    """Integration tests for AutofixCoordinator."""
    ...
    def test_error_recovery(self, coordinator):
        ...
        with patch.object(coordinator, '_run_fix_command', side_effect=Exception("Test error")):
            result = coordinator.apply_autofix_for_hooks("fast", hook_results)
