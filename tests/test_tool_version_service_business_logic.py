...
class TestSubprocessExecutionEdgeCases:
    ...
    def test_tool_not_installed_graceful_handling(self, service: ToolVersionService) -> None:
        """Test graceful handling when tools are not installed."""
        with patch('subprocess.run', side_effect=FileNotFoundError("ruff command not found")):
            result = service._get_ruff_version()
    ...
    def test_tool_returns_malformed_version_output(self, service: ToolVersionService) -> None:
    ...
    def test_tool_version_parsing_with_complex_output(self, service: ToolVersionService) -> None:
    ...
    def test_subprocess_non_zero_exit_code_handling(self, service: ToolVersionService) -> None:
...
class TestNetworkResilienceAndErrorRecovery:
    ...
    @pytest.mark.asyncio
    async def test_http_error_status_recovery(self, service: ToolVersionService) -> None:
    ...
    @pytest.mark.asyncio
    async def test_malformed_json_response_recovery(self, service: ToolVersionService) -> None:
    ...
    @pytest.mark.asyncio
    async def test_missing_version_in_response(self, service: ToolVersionService) -> None:
...
class TestVersionComparisonAlgorithm:
    ...
    def test_semantic_version_comparison(self, service: ToolVersionService) -> None:
        ...
        for current, latest, expected in test_cases:
            ...
            assert result == expected, f"Failed comparison: {current} vs {latest}, got {result}, expected {expected}"
    ...
    def test_invalid_version_format_handling(self, service: ToolVersionService) -> None:
        ...
        for current, latest in invalid_cases:
            ...
            result = service._version_compare(current, latest)
            assert result == 0, f"Invalid version comparison failed: {current} vs {latest}"
...
class TestFullWorkflowIntegration:
    ...
    @pytest.mark.asyncio
    async def test_check_single_tool_success_workflow(self, service: ToolVersionService) -> None:
    ...
    @pytest.mark.asyncio
    async def test_check_single_tool_missing_tool_workflow(self, service: ToolVersionService) -> None:
    ...
    @pytest.mark.asyncio
    async def test_check_single_tool_error_workflow(self, service: ToolVersionService) -> None:
    ...
    @pytest.mark.asyncio
    async def test_check_all_tools_comprehensive(self, service: ToolVersionService) -> None:
        ...
        def mock_version_getter(tool_name: str):
            def getter():
                ...
                with patch('subprocess.run', return_value=mock_result):
                    method = getattr(service, f"_get_{tool_name.replace('-', '')}_version")
                    return method()
            return getter
...
class TestVersionInfoCreationLogic:
    ...
    def test_installed_version_info_with_update(self, service: ToolVersionService) -> None:
    ...
    def test_installed_version_info_no_update(self, service: ToolVersionService) -> None:
    ...
    def test_installed_version_info_network_failure(self, service: ToolVersionService) -> None:
...
class TestConcurrentToolChecking:
    ...
    @pytest.mark.asyncio
    async def test_concurrent_tool_checks_isolation(self, service: ToolVersionService) -> None:
    ...
    @pytest.mark.asyncio
    async def test_network_fetch_timeout_configuration(self, service: ToolVersionService) -> None:
...
class TestRealWorldPerformanceScenarios:
    ...
    @pytest.mark.asyncio
    async def test_mixed_success_failure_scenario(self, service: ToolVersionService) -> None:
        ...
        with patch.object(service, '_fetch_latest_version', side_effect=mock_fetch_latest):
            results = await service.check_tool_updates()
