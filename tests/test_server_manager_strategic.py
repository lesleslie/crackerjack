...
class TestServerManagement:
    ...
    def test_stop_mcp_server_success(self, mock_stop_process, mock_find_processes):
        ...
        mock_find_processes.return_value = [
            ...
            {'pid': 5678, 'command': 'python -m crackerjack --start-mcp-server --websocket-port 8675'},
        ]
        ...
        assert mock_console.print.call_count >= 4  # Start and success messages for each process
    ...
    def test_stop_mcp_server_no_processes(self, mock_find_processes):
        ...
        assert result is True
        mock_console.print.assert_called_with("[yellow]⚠️ No MCP server processes found[/yellow]")

    @patch('crackerjack.services.server_manager.find_mcp_server_processes')
    @patch('crackerjack.services.server_manager.stop_process')
    def test_stop_mcp_server_partial_failure(self, mock_stop_process, mock_find_processes):
    ...
    @patch('crackerjack.services.server_manager.stop_process')
    def test_stop_websocket_server_success(self, mock_stop_process, mock_find_processes):
...
class TestServerRestart:
    ...
    def test_restart_mcp_server_success(self, mock_sleep, mock_popen, mock_stop):
        ...
        cmd = args[0]
        assert cmd == ['/usr/bin/python3', '-m', 'crackerjack', '--start-mcp-server', '--websocket-port', '8675']

    @patch('crackerjack.services.server_manager.stop_mcp_server')
    @patch('subprocess.Popen')
    @patch('time.sleep')
    def test_restart_mcp_server_no_websocket_port(self, mock_sleep, mock_popen, mock_stop):
    ...
    def test_restart_mcp_server_popen_failure(self, mock_sleep, mock_popen, mock_stop):
        ...
        assert result is False
        mock_console.print.assert_any_call("❌ Failed to restart MCP server: Failed to start process")

    @patch('crackerjack.services.server_manager.Console')
    @patch('crackerjack.services.server_manager.stop_mcp_server')
    @patch('subprocess.Popen')
    @patch('time.sleep')
    def test_restart_mcp_server_default_console(self, mock_sleep, mock_popen, mock_stop, mock_console_class):
...
class TestServerStatus:
    ...
    @patch('crackerjack.services.server_manager.find_mcp_server_processes')
    @patch('crackerjack.services.server_manager.find_websocket_server_processes')
    def test_list_server_status_with_processes(self, mock_find_websocket, mock_find_mcp):
    ...
    def test_list_server_status_no_processes(self, mock_find_websocket, mock_find_mcp):
        ...
        print_calls = [call[0][0] for call in mock_console.print.call_args_list]
        assert any("No crackerjack servers currently running" in call for call in print_calls)

    @patch('crackerjack.services.server_manager.Console')
    @patch('crackerjack.services.server_manager.find_websocket_server_processes')
    @patch('crackerjack.services.server_manager.find_mcp_server_processes')
    def test_list_server_status_default_console(self, mock_find_mcp, mock_find_websocket, mock_console_class):
...
class TestEdgeCasesAndIntegration:
    ...
    def test_process_discovery_with_different_commands(self, mock_run):
        ...
        processes = find_mcp_server_processes()
        assert len(processes) == 3  # All lines contain both 'crackerjack' and '--start-mcp-server'
    ...
    def test_status_output_formatting(self, mock_find_websocket, mock_find_mcp):
        ...
        print_calls = [call[0][0] for call in mock_console.print.call_args_list]
        cpu_mem_call = next((call for call in print_calls if "CPU: 15.7%" in call and "Memory: 8.3%" in call), None)
        assert cpu_mem_call is not None
        
        command_call = next((call for call in print_calls if "--websocket-port 8675 --debug" in call), None)
