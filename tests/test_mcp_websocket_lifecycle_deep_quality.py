...
class TestWebSocketConnectionManagement:
    ...
    @pytest.mark.asyncio
    async def test_broadcast_to_healthy_connections(self, job_manager: JobManager) -> None:
    ...
    @pytest.mark.asyncio
    async def test_broadcast_with_failed_connections(self, job_manager: JobManager) -> None:
...
class TestJobProgressFileManagement:
    ...
    def test_get_job_progress_success(self, job_manager: JobManager, temp_progress_dir: Path) -> None:
    ...
    def test_get_job_progress_corrupted_json(self, job_manager: JobManager, temp_progress_dir: Path) -> None:
    ...
    def test_get_job_progress_empty_file(self, job_manager: JobManager, temp_progress_dir: Path) -> None:
    ...
    def test_get_job_progress_permission_error(self, job_manager: JobManager, temp_progress_dir: Path) -> None:
    ...
    def test_get_latest_job_id_multiple_files(self, job_manager: JobManager, temp_progress_dir: Path) -> None:
...
class TestNewJobDetectionAndProcessing:
    ...
    @pytest.mark.asyncio
    async def test_process_new_job_detection(self, job_manager: JobManager, temp_progress_dir: Path) -> None:
    ...
    @pytest.mark.asyncio
    async def test_process_invalid_progress_file(self, job_manager: JobManager, temp_progress_dir: Path) -> None:
    ...
    @pytest.mark.asyncio
    async def test_process_already_known_job(self, job_manager: JobManager, temp_progress_dir: Path) -> None:
...
class TestConcurrentJobManagement:
    ...
    @pytest.mark.asyncio
    async def test_concurrent_connection_management(self, job_manager: JobManager) -> None:
    ...
    async def test_concurrent_broadcasting(self, job_manager: JobManager) -> None:
        ...
        jobs_and_websockets = [
            (f"job-{i}", [AsyncMock() for _ in range(3)])
            for i in range(5)
        ]
        ...
        await asyncio.gather(*[
            broadcast_to_job(job_id) 
            for job_id, _ in jobs_and_websockets
        ])
    ...
    async def test_concurrent_job_validation(self, job_manager: JobManager) -> None:
        ...
        job_ids = [
            str(uuid.uuid4()) for _ in range(10)  # Valid UUIDs
        ] + [
            f"job-{i}" for i in range(10)  # Valid alphanumeric
        ] + [
            f"../invalid-{i}" for i in range(5)  # Invalid paths
        ]
...
class TestJobManagerResourceManagement:
    ...
    def test_memory_usage_with_many_connections(self, job_manager: JobManager) -> None:
        ...
        total_connections = sum(len(connections) for connections in job_manager.active_connections.values())
    ...
    def test_known_jobs_memory_management(self, job_manager: JobManager, temp_progress_dir: Path) -> None:
