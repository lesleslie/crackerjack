...
class TestHookLockManagerBasics:
    ...
    def test_lock_stats_structure(self):
        ...
        if "complexipy" in stats:
            ...
            required_fields = {
                "total_acquisitions", "avg_wait_time", "max_wait_time", "min_wait_time",
                "avg_execution_time", "max_execution_time", "min_execution_time",
                "currently_locked", "lock_failures", "timeout_failures", "success_rate", "lock_timeout"
            }
...
class TestGlobalLockFileOperations:
    ...
    async def test_global_lock_file_creation(self, tmp_path):
        ...
        async with manager.acquire_hook_lock(hook_name):
            ...
            required_fields = {"session_id", "hostname", "pid", "hook_name", "acquired_at", "last_heartbeat"}
            for field in required_fields:
...
class TestHeartbeatMechanism:
    ...
    async def test_heartbeat_task_cleanup_on_release(self, tmp_path):
        ...
        test_config = GlobalLockConfig(
            lock_directory=tmp_path / "locks",
            session_heartbeat_interval=0.1
        )
        manager._global_config = test_config
        manager.enable_global_lock(True)
...
class TestStatisticsTracking:
    ...
    async def test_global_lock_stats_tracking(self, tmp_path):
        ...
        initial_attempts = initial_stats["statistics"].get(hook_name, {}).get("attempts", 0)
        initial_successes = initial_stats["statistics"].get(hook_name, {}).get("successes", 0)
    ...
    def test_comprehensive_status_includes_global_stats(self, tmp_path):
        ...
        expected_config_fields = {
            "timeout_seconds", "stale_lock_hours", "heartbeat_interval",
            "max_retry_attempts", "retry_delay_seconds", "enable_lock_monitoring"
        }
...
class TestConfigurationIntegration:
    ...
    def test_configure_from_options(self, tmp_path):
        ...
        assert str(manager._global_config.lock_directory) == mock_options.global_lock_dir
