...
class TestToolVersionServiceSimpleWorking:
    ...
    def test_service_initialization_basic(self, service: ToolVersionService, console: Console) -> None:
    ...
    def test_get_ruff_version_basic(self, mock_run: Mock, service: ToolVersionService) -> None:
    ...
    @patch('subprocess.run')
    def test_get_ruff_version_error(self, mock_run: Mock, service: ToolVersionService) -> None:
    ...
    @patch('subprocess.run')
    def test_get_pyright_version_basic(self, mock_run: Mock, service: ToolVersionService) -> None:
    ...
    @patch('subprocess.run')
    def test_get_precommit_version_basic(self, mock_run: Mock, service: ToolVersionService) -> None:
    ...
    @patch('subprocess.run')
    def test_get_uv_version_basic(self, mock_run: Mock, service: ToolVersionService) -> None:
    ...
    async def test_check_single_tool_missing(self, service: ToolVersionService) -> None:
        ...
        with patch.object(service.console, 'print'):
            result = await service._check_single_tool("missing-tool", mock_version_getter)
    ...
    @pytest.mark.asyncio
    async def test_fetch_latest_version_if_available(self, service: ToolVersionService) -> None:
        ...
        try:
            ...
            with patch('aiohttp.ClientSession') as mock_session_class:
                ...
                mock_response = Mock()
                mock_response.json = AsyncMock(return_value={"info": {"version": "1.0.5"}})
    ...
    @patch('subprocess.run')
    def test_all_version_methods_exist_and_callable(self, mock_run: Mock, service: ToolVersionService) -> None:
    ...
    @patch('subprocess.run')
    def test_subprocess_error_handling_comprehensive(self, mock_run: Mock, service: ToolVersionService) -> None:
        ...
        for error in errors:
            ...
            result = service._get_ruff_version()
            assert result is None, f"Should return None for error: {type(error).__name__}"
    ...
    def test_integration_attempt_without_mocks(self, service: ToolVersionService) -> None:
    ...
    @patch('subprocess.run')
    def test_version_parsing_patterns(self, mock_run: Mock, service: ToolVersionService) -> None:
    ...
    def test_console_integration(self, service: ToolVersionService) -> None:
        ...
        with patch.object(service.console, 'print') as mock_print:
            info = service._create_missing_tool_info("test-tool")
