...
class TestSessionCoordinator:
    ...
    def test_finalize_session(self, coordinator):
...
class TestAutofixCoordinator:
    ...
    def test_apply_fast_stage_fixes(self, coordinator):
    ...
    def test_validate_fix_command(self, coordinator):
        ...
        # Valid commands
        assert coordinator.validate_fix_command(["uv", "run", "ruff", "format", "."]) is True
        assert coordinator.validate_fix_command(["uv", "run", "bandit", "-f", "json"]) is True
    ...
    def test_should_skip_autofix(self, coordinator):
        ...
        results_with_import_error = [MockResult("ModuleNotFoundError: No module named 'test'")]
...
class TestPhaseCoordinator:
    ...
    def test_run_configuration_phase_failure(self, coordinator):
    ...
    def test_run_fast_hooks_only(self, coordinator, mock_dependencies):
        """Test fast hooks execution."""
        options = MockOptions()
        
        # Mock successful hook execution
        mock_dependencies['hook_manager'].run_fast_hooks.return_value = [
            HookResult(id="1", name="test", status="passed", duration=1.0, issues_found=[], stage="fast")
        ]
    ...
    def test_run_comprehensive_hooks_only(self, coordinator, mock_dependencies):
        """Test comprehensive hooks execution."""
        options = MockOptions()
        
        # Mock successful hook execution
        mock_dependencies['hook_manager'].run_comprehensive_hooks.return_value = [
            HookResult(id="1", name="test", status="passed", duration=1.0, issues_found=[], stage="comprehensive")
        ]
    ...
    def test_run_testing_phase_success(self, coordinator, mock_dependencies):
        ...
        assert result is True
        mock_dependencies['test_manager'].run_tests.assert_called_once()
    
    def test_run_testing_phase_failure(self, coordinator, mock_dependencies):
    ...
    def test_run_publishing_phase_disabled(self, coordinator):
    ...
    def test_run_publishing_phase_enabled(self, coordinator, mock_dependencies):
        """Test publishing phase when enabled."""
        options = MockOptions(publish=True)
        
        # Mock successful publishing
        mock_dependencies['publish_manager'].publish.return_value = True
        
        result = coordinator.run_publishing_phase(options)
        assert result is True
        mock_dependencies['publish_manager'].publish.assert_called_once()
    
    def test_run_commit_phase_disabled(self, coordinator):
    ...
    def test_run_commit_phase_enabled(self, coordinator, mock_dependencies):
        """Test commit phase when enabled."""
        options = MockOptions(commit=True)
        
        # Mock successful commit
        mock_dependencies['git_service'].commit_changes.return_value = True
        
        result = coordinator.run_commit_phase(options)
        assert result is True
        mock_dependencies['git_service'].commit_changes.assert_called_once()
...
class TestCoreModuleIntegration:
    ...
    def test_session_and_autofix_integration(self, console, temp_path):
    ...
    def test_mock_workflow_pipeline(self, console, temp_path):
        ...
        assert autofix.validate_fix_command(["uv", "run", "ruff", "format", "."]) is True
