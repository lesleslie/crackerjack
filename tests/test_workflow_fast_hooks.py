...
def test_direct_crackerjack_behavior():
    ...
    has_fast_hooks = any(phrase in output.lower() for phrase in [
        "fast hooks", "running code quality checks", "🔍 hooks"
    ])
    
    has_tests = any(phrase in output.lower() for phrase in [
        "🧪 tests", "running test suite", "pytest", "test execution"
    ])
    
    has_comprehensive = any(phrase in output.lower() for phrase in [
        "comprehensive", "pre-push", "pyright", "bandit"
    ])
    
    print(f"Analysis: fast_hooks={has_fast_hooks}, tests={has_tests}, comprehensive={has_comprehensive}")
    ...
    if result.returncode == 0:  # Fast hooks passed
        ...
        if result2.returncode != 0:
            print("✅ PASS: Fast hooks failed and overall workflow failed appropriately")
            return True
        else:
...
def test_workflow_with_external_project():
    ...
    try:
        ...
        subprocess.run(["git", "commit", "-m", "initial"], capture_output=True, check=False)
        ...
        workflow_attempted_tests = any(phrase in output.lower() for phrase in [
            "test", "pytest", "no tests ran", "collected 0 items"
        ])
        
        print(f"Analysis: has_hooks={has_hooks}, attempted_tests={workflow_attempted_tests}")
...
def check_phase_coordinator_implementation():
    ...
    try:
        ...
        try:
            ...
            returns_bool = any(phrase in source for phrase in [
            ...
            if returns_bool:
                ...
                print("❌ FAIL: run_fast_hooks_only method may not return proper boolean")
                print("Method source:")
                print(source)
                return False
...
def main():
    ...
    print(f"Test 1 (Direct workflow behavior): {'✅ PASS' if test1_result else '❌ FAIL'}")
    print(f"Test 2 (Clean project workflow): {'✅ PASS' if test2_result else '❌ FAIL'}")
    print(f"Test 3 (Bad formatting project): {'✅ PASS' if test3_result else '❌ FAIL'}")
