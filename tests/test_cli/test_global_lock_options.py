...
class TestGlobalLockCLIOptions:
    ...
    def test_global_lock_cli_options_exist(self):
        ...
        for option_name in expected_options:
            ...
            assert isinstance(CLI_OPTIONS[option_name], typer.Option)
    ...
    def test_global_lock_cleanup_option_definition(self):
        ...
        assert "--cleanup-stale-locks" in str(option) or "--no-cleanup-stale-locks" in str(option)
...
class TestConfigurationFlow:
    ...
    def test_cli_to_lock_manager_configuration_flow(self, tmp_path):
        ...
        assert lock_manager._global_config.lock_directory == tmp_path / "full_flow_locks"
    ...
    def test_custom_directory_configuration_flow(self, tmp_path):
        ...
        for custom_dir in custom_dirs:
            cli_options = Options(
                disable_global_locks=False,
                global_lock_dir=str(custom_dir)
            )
...
class TestCLIArgumentValidation:
    ...
    def test_boolean_option_validation(self):
        ...
        for disable_locks, cleanup in boolean_combinations:
            options = Options(
                disable_global_locks=disable_locks,
                global_lock_cleanup=cleanup
            )
...
class TestCLIOptionCompletion:
    ...
    def test_cli_options_dict_completeness(self):
        ...
        for option_name in expected_cli_options:
            ...
            assert isinstance(option_def, typer.Option)
    ...
    def test_options_model_protocol_compliance(self):
        ...
        def test_function(opts: OptionsProtocol) -> bool:
            return (
                hasattr(opts, "disable_global_locks") and
                hasattr(opts, "global_lock_timeout") and 
                hasattr(opts, "global_lock_cleanup") and
                hasattr(opts, "global_lock_dir")
            )
...
class TestCLIErrorHandling:
    ...
    def test_type_consistency(self):
        ...
        assert options_defaults.disable_global_locks == cli_option_defaults["disable_global_locks"]
        assert options_defaults.global_lock_timeout == cli_option_defaults["global_lock_timeout"]
        assert options_defaults.global_lock_cleanup == cli_option_defaults["global_lock_cleanup"]
        assert options_defaults.global_lock_dir == cli_option_defaults["global_lock_dir"]
