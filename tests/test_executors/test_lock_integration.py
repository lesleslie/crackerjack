...
class TestAsyncHookExecutorIntegration:
    ...
    async def test_async_executor_with_global_locks_enabled(self, tmp_path):
        ...
        test_config = GlobalLockConfig(
            lock_directory=tmp_path / "executor_locks",
            enabled=True
        )
    ...
    async def test_async_executor_concurrent_prevention(self, tmp_path):
        ...
        lock_manager = HookLockManager()
        test_config = GlobalLockConfig(
            lock_directory=tmp_path / "concurrent_locks",
            enabled=True
        )
        lock_manager._global_config = test_config
        lock_manager.enable_global_lock(True)
        ...
        success_count = sum(1 for r in results if isinstance(r, type(results[0])) and r.success)
...
class TestIndividualHookExecutorIntegration:
    ...
    async def test_individual_executor_with_global_locks(self, tmp_path):
        ...
        lock_manager = HookLockManager()
        test_config = GlobalLockConfig(
            lock_directory=tmp_path / "individual_locks",
            enabled=True
        )
        lock_manager._global_config = test_config
        lock_manager.enable_global_lock(True)
...
class TestHookExecutorLockCoordination:
    ...
    async def test_cross_executor_lock_coordination(self, tmp_path):
        ...
        individual_strategy = HookStrategy(name="individual_strategy", hooks=[individual_hook])
    ...
    async def test_executor_global_lock_statistics(self, tmp_path):
        ...
        initial_attempts = initial_stats["statistics"].get(stats_hook, {}).get("attempts", 0)
        ...
        updated_attempts = updated_stats["statistics"].get(stats_hook, {}).get("attempts", 0)
...
class TestExecutorConfigurationFlow:
    ...
    def test_lock_manager_configuration_from_options(self, tmp_path):
        ...
        assert str(lock_manager._global_config.lock_directory) == mock_options.global_lock_dir
    ...
    def test_custom_timeout_configuration(self, tmp_path):
        ...
        assert executor.hook_lock_manager._global_config.timeout_seconds == float(custom_timeout)
...
class TestExecutorLockManagerMocking:
    ...
    def test_mock_lock_manager_configuration(self):
        ...
        for scenario in test_scenarios:
            ...
            mock_lock_manager.is_global_lock_enabled.return_value = scenario["global_enabled"]
            ...
            with tempfile.TemporaryDirectory() as tmp_dir:
                ...
                assert executor.hook_lock_manager is mock_lock_manager
                assert executor.hook_lock_manager.requires_lock("test") == scenario["requires_lock"]
                assert executor.hook_lock_manager.is_global_lock_enabled() == scenario["global_enabled"]