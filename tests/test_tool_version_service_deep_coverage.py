...
class TestToolVersionServiceDeepCoverage:
    ...
    def test_service_initialization_comprehensive(self, service: ToolVersionService, console: Console) -> None:
    ...
    def test_all_version_methods_comprehensive(self, mock_run: Mock, service: ToolVersionService) -> None:
        ...
        for method_name, stdout, expected in test_cases:
            ...
            result = method()
            assert expected in result, f"{method_name} failed: got {result}, expected {expected}"

    @patch('subprocess.run')
    def test_version_detection_error_handling(self, mock_run: Mock, service: ToolVersionService) -> None:
        ...
        for method in methods_to_test:
            result = method()
            assert result == "unknown", f"Method {method.__name__} should return 'unknown' on error"

    @patch('subprocess.run')
    def test_version_detection_with_complex_output(self, mock_run: Mock, service: ToolVersionService) -> None:
        """Test version detection with complex output formats."""
        complex_outputs = [
            ("ruff 0.1.6\nCompiled with Rust 1.75.0", service._get_ruff_version, "0.1.6"),
            ("pyright 1.1.365\nNode.js v18.17.0", service._get_pyright_version, "1.1.365"),
            ("pre-commit 3.5.0\nPython 3.11.0", service._get_precommit_version, "3.5.0"),
            ("uv 0.2.18 (Cargo 1.75.0)\nInstalled via pip", service._get_uv_version, "0.2.18"),
        ]
    ...
    def test_version_comparison_comprehensive(self, service: ToolVersionService) -> None:
    ...
    def test_version_info_creation_methods(self, service: ToolVersionService) -> None:
        ...
        info_update = service._create_installed_version_info("test-tool", "1.0.0", "1.1.0")
        ...
        info_no_update = service._create_installed_version_info("test-tool", "1.1.0", "1.1.0")
        assert info_no_update.update_available is False

        # Test installed version info without latest version
        info_no_latest = service._create_installed_version_info("test-tool", "1.0.0", None)
    ...
    @patch('subprocess.run')
    def test_check_installed_tools_comprehensive_workflow(self, mock_run: Mock, service: ToolVersionService) -> None:
        ...
        assert all(isinstance(info, VersionInfo) for info in results), "All results should be VersionInfo instances"
        assert all(info.error is None for info in results), "No errors should occur with successful mocks"
        ...
        for info in results:
            assert info.current_version != "unknown", f"Tool {info.tool_name} should have extracted version"

    @patch('subprocess.run')
    def test_check_installed_tools_mixed_results(self, mock_run: Mock, service: ToolVersionService) -> None:
        ...
        responses = [
            ...
            subprocess.CalledProcessError(127, ["uv", "--version"]),      # uv fail (command not found)
        ]
        mock_run.side_effect = responses
        ...
        assert len(results) == 4, "Should return results for all tools, even failed ones"
        ...
        assert successful_tools == expected_successful, f"Expected successful: {expected_successful}"
        ...
        for failed_result in failed_results:
            assert "not installed" in failed_result.error, f"Failed result should indicate not installed"
    ...
    def test_performance_characteristics(self, service: ToolVersionService) -> None:
        ...
        with patch('subprocess.run') as mock_run:
            ...
            assert execution_time < 10.0, f"Performance test took {execution_time}s, should be under 10s"
    ...
    def test_memory_usage_patterns(self, service: ToolVersionService) -> None:
        ...
        with patch('subprocess.run') as mock_run:
            ...
            for results in results_list:
                assert all(r.tool_name in {"ruff", "pyright", "pre-commit", "uv"} for r in results)
    ...
    def test_error_scenarios_comprehensive(self, service: ToolVersionService) -> None:
        ...
        error_types = [
            ...
            subprocess.CalledProcessError(127, ["tool", "--version"]),  # Command not found
        ...
        for error in error_types:
            with patch('subprocess.run', side_effect=error):
                ...
                result = service._get_ruff_version()
                assert result == "unknown", f"Should return 'unknown' for error: {type(error).__name__}"
    ...
    def test_integration_with_real_environment(self, service: ToolVersionService) -> None:
