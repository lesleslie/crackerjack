...
# TestAttemptCoverageImprovement was removed in this commit. The
# ``crackerjack.orchestration.coverage_improvement`` module was deleted
# by c0f73b26 (2025-12-27) "feat(migration): Complete Phase 2 - Remove
# ACB Dependency" — see workflow_executor.py:573 for the live stub which
# always returns ``{"status": "skipped"}``. The deleted tests patched the
# missing module into ``sys.modules`` to exercise an orchestrator that no
# longer exists; reinstating them would require re-creating the deleted
# module first.
