...
def test_fast_hooks_failure_stops_workflow():
    ...
    fast_hooks_failed = any(phrase in output.lower() for phrase in [
        "fast hooks failed",
        "fast hooks: failed",
        "failed, 0 errors", 
        "format check failed",
...
def test_fast_hooks_success_continues_workflow():
    ...
    fast_hooks_passed = any(phrase in output.lower() for phrase in [
        "fast hooks passed",
        "fast hooks: passed", 
        "✅ fast hooks",
    ])
    ...
    tests_run = any(phrase in output.lower() for phrase in [
        "running test suite",
        "tests passed",
        "pytest",
        "test execution"
...
def main():
    ...
    try:
        ...
        print(f"Test 1 (Fast hooks failure stops): {'✅ PASS' if test1_result else '❌ FAIL'}")
        print(f"Test 2 (Fast hooks success continues): {'✅ PASS' if test2_result else '❌ FAIL'}")
        print(f"Test 3 (Workflow order correct): {'✅ PASS' if test3_result else '❌ FAIL'}")
