...
class TestBigLebowskiVictory:
    ...
    def test_crackerjack_facade_process_success(self):
        ...
        with patch.object(facade.orchestrator, 'run_complete_workflow', return_value=True):
    ...
    def test_crackerjack_facade_process_failure(self):
        ...
        with patch.object(facade.orchestrator, 'run_complete_workflow', return_value=False):
            try:
                facade.process(mock_options)
    ...
    def test_crackerjack_facade_keyboard_interrupt(self):
        ...
        with patch.object(facade.orchestrator, 'run_complete_workflow', side_effect=KeyboardInterrupt):
            try:
                facade.process(mock_options)
                assert False, "Should have raised SystemExit"
            except SystemExit as e:
