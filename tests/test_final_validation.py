...
def test_fast_hooks_failure_behavior():
    ...
    try:
        ...
        subprocess.run(["git", "commit", "-m", "initial with bad formatting"], capture_output=True, check=True)
        ...
        if fast_hooks_fail and result.returncode != 0 and not tests_attempted:
            ...
            print("⚠️  PARTIAL: Fast hooks failed, workflow failed, but tests were still attempted")
...
def test_implementation_directly():
    ...
    try:
        ...
        if return_annotation == bool:
            ...
            print(f"❌ run_fast_hooks_only return annotation is {return_annotation}, should be bool")
            return False
        ...
        if has_boolean_returns:
            ...
            print("❌ run_fast_hooks_only implementation doesn't clearly return boolean")
            print("Source:")
            print(source)
            return False
        ...
        if "if not self.phases.run_fast_hooks_only(options):" in workflow_source:
            print("✅ Workflow correctly checks boolean return from run_fast_hooks_only")
        else:
...
def main():
    ...
    print(f"Behavioral test (fast hooks stop workflow): {'✅ PASS' if behavior_test else '❌ FAIL'}")
    print(f"Implementation test (boolean returns): {'✅ PASS' if implementation_test else '❌ FAIL'}")
    ...
    if behavior_test and implementation_test:
        ...
        if not behavior_test:
            print("- Behavioral issue: Fast hooks may not be stopping workflow properly")
        if not implementation_test:
            print("- Implementation issue: Methods may not be returning correct boolean values")
        return 1
