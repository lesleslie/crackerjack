...
class RateLimiter:
    ...
    async def __call__(self, request: Request, call_next: Callable):
        ...
        response.headers["X-RateLimit-Reset"] = str(
            int(time.time()) + self.redis.ttl(key)
        )
...
async def get_user(
    user_id: int, db: Session = Depends(get_db), token: dict = Depends(verify_token)
):
    """Get user by ID"""
    service = UserService(db)
...
if __name__ == "__main__":
    ...
    uvicorn.run("app.main:app", host=settings.SERVER_HOST, port=8000, reload=True)
```
