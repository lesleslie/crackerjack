...
2. **subprocess timeout** (10 minutes): Hardcoded in `test_executor.py:23,43,158`
   ```python
   # BEFORE (BROKEN):
...
2. Line 43: `timeout: int = 600` → `timeout: int = 1800`
3. Line 393: `timeout: int` → `timeout: int = 1800` (added default)
...
2. **No premature timeouts**: Subprocess wrapper no longer kills tests before pytest's timeout
3. **Proper timeout hierarchy**: pytest-timeout (1800s) > subprocess timeout (1800s)
4. **Better debugging**: Test failures are now actual test failures, not wrapper timeouts
