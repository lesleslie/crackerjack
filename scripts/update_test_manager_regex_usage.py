...
old_metric = r'metric_match = re\.search\(metric_pattern, summary_text, re\.IGNORECASE\)'
...
old_coverage = r'test_path = re\.sub\(r"\\s\*\\\[\\s\\d\+\%\)\\\$", "", test_path\)\.strip\(\)'
...
old_location = r'location_match = re\.match\(r"\^\(\.\+?\\.py\):\(\\d\+\):\\s\*\(\.\*\)\$", line\)'
...
old_summary_failure = r'match = re\.match\(r"\^FAILED\\s\+\(\.\+?\)\\s\+-\\s\+\(\.\+\)\$", line\)'
...
old_ellipsis = r'error_message = re\.sub\(r"\\\.\\\.\\\$", "", error_message\)\.strip\(\)'
