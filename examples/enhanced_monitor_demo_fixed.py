...
async def main():
    parser = argparse.ArgumentParser(description = "Enhanced Progress Monitor Demo (Fixed)")
    parser.add_argument(" -- no - clear", action = "store_true", help = "Don't clear terminal")
    parser.add_argument(" -- refresh", type = float, default = 0.5, help = "Refresh interval")
    parser.add_argument(" -- no - watchdog", action = "store_true", help = "Disable watchdog services")
    parser.add_argument(" -- no - textual", action = "store_true", help = "Use Rich instead of Textual")
