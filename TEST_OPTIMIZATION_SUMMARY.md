...
2. Adjusted timeout values from 0.1s → 0.05s to maintain test logic
...
2. **Sleep accumulation is costly** - 1,109.5s of sleep across 102 calls
3. **Benchmark markers are critical** - performance tests shouldn't run on every execution
4. **Test logic integrity matters** - timeout margins must be maintained for reliable tests
5. **Quick wins exist** - 177s saved with ~1 hour of focused work
...
3. `tests/unit/core/test_resource_manager.py` - Reduce sleeps
4. `tests/unit/core/test_timeout_manager.py` - Reduce sleeps + timeout adjustments
5. `tests/test_profiler_integration.py` - Reduce sleeps
