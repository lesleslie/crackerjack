...
2. Architecture changed from flat attributes to nested config objects
...
2. **Simplified Adapters**: pyscn adapter simplified from JSON to text-only output
3. **Mock Object Patterns**: Tests using class attributes instead of instance attributes break `vars()` inspection

### Common Fix Patterns
1. **Test Expectation Updates**: Most fixes just need updated assertions, not implementation changes
2. **Property Access**: Use property methods (`.strip_code`) instead of direct attribute access (`.clean`)
3. **Instance vs Class Attributes**: Mock objects need `__init__` to set instance attributes
...
2. Address mock object patterns in other tests
3. Update assertion patterns for nested configs

### Core Fixes (2-4 hours):
4. Investigate SessionCoordinator `session_tracker` initialization
5. Fix Security Service secret detection
6. Decide on Code Cleaner pattern registry fix vs. test update
