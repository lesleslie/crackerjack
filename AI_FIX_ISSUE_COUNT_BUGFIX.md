...
3. **Hook Count Mystery**: The hook reports `issues=60`
...
2. `_parse_hook_results_to_issues()` combines `output + error + error_message`
3. `_parse_hook_to_issues()` routes to `_parse_type_checker_output()` for zuban
4. Lines split by `\n` and filtered by `_should_parse_line()`
5. Each valid line parsed by `_parse_type_checker_line()` into an Issue
...
2. **Worktree Handling**: Decide if worktree errors should be included in AI-fix
3. **Count Consistency**: Align hook issue counting with parser extraction
