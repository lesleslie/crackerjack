...
1. `__post_init__()` copies `output` to `error_message`
1. Autofix coordinator's `_extract_raw_output()` combines: `output + error + error_message`
1. Result: JSON + empty + JSON = **duplicated JSON**
1. Parser sees "Extra data" error at position 16921 (after first JSON ends)
...
1. **factory.py** - ParserFactory with validation and caching
1. **json_parsers.py** - 7 JSON parser implementations
1. **regex_parsers.py** - Legacy regex parsers for non-JSON tools
...
1. **Immediately after reading** - Remove JSON files right after parsing
1. **Failsafe** - Can be called at end as final cleanup
...
1. **JSON_PARSER_IMPLEMENTATION_COMPLETE.md** - Implementation details
1. **PARSING_FIX_SUMMARY.md** - Summary of all parsing fixes
1. **JSON_PARSING_ARCHITECTURE.md** - Architecture overview
1. **JSON_PARSING_IMPLEMENTATION.md** - Implementation guide
1. **JSON_PARSING_PERFORMANCE.md** - Performance benchmarks
1. **JSON_PARSING_SUMMARY.md** - Executive summary
...
1. ❌ Auto-copying output to error_message
1. ❌ Uncleaned temporary files
1. ❌ JSON lines counted as issues

### Added Best Practices

1. ✅ Protocol-based parser design
1. ✅ Centralized parser factory
1. ✅ File-based JSON support
1. ✅ Automatic temporary file cleanup
1. ✅ Count validation for all parsers
...
1. ✅ Verify all JSON parsers work in production
1. ✅ Monitor for any remaining parsing issues

### Short-term

1. Add JSON parsers for remaining tools (refurb, codespell if supported)
1. Improve error messages for non-JSON tools
1. Add integration tests for parser factory

### Long-term

1. Target 50%+ JSON parser coverage
1. Migrate all regex parsers to JSON where possible
1. Add parser performance monitoring
...
1. Understanding data flow through the system
1. Testing with real data, not just unit tests
