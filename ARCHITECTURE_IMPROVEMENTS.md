...
2. CLI sets `AI_AGENT=1` environment variable
3. `DefaultAdapterFactory._enable_tool_native_fixes()` detects AI_AGENT
4. Enables `fix_enabled=True` in adapter settings (ruff, etc.)
5. Hooks run with auto-fix enabled during normal execution
6. Only unfixed issues are reported to AI agents

**Benefits**:
1. ✅ **Faster**: No need to re-run hooks multiple times
2. ✅ **More reliable**: Use each tool's own fixing logic
3. ✅ **Simpler**: Hook reports only what it couldn't auto-fix
4. ✅ **Native capabilities**: Leverage each tool's strengths
...
5. **Extend RefactoringAgent** (2-3 hours)
...
2. **Reliable fixes**: Each tool knows best how to fix its issues
3. **Simplified workflow**: No need to re-run hooks multiple times
4. **AST-based transformations**: PatternAgent handles complex refactorings
