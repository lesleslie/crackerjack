...
2. **`crackerjack/utils/dependency_guard.py`**
...
2. ✅ Running `python -m crackerjack` produces NO output to stderr (silent)
3. ✅ Running `python -m crackerjack --verbose` shows NO low-level logs (user detail only on stdout)
4. ✅ Running `python -m crackerjack --verbose` produces NO output to stderr (silent)
5. ✅ Running `python -m crackerjack --debug` shows ALL logging output on stdout
6. ✅ Running `python -m crackerjack --debug` produces structured JSON logs to stderr ONLY
7. ✅ Progress bars update on single line (no repetition)
8. ✅ Progress bars clear after completion (transient behavior)
9. ✅ All existing tests pass
10. ✅ CI/CD pipeline output remains readable (stdout for humans, stderr for machines)
11. ✅ External projects (session-mgmt-mcp) benefit from fixes
...
2. Restore print statements in `dependency_guard.py`
3. Revert `transient=True` → `transient=False` change
4. Git commit: `git revert <commit-hash>`
...
2. **Review with specialized agents** (code-reviewer, python-pro)
3. **Implement Phase 1** (logging fixes)
4. **Test logging behavior** across all flag combinations
5. **Implement Phase 2** (progress bar fixes)
6. **Test progress bar behavior** across workflows
7. **Run full quality workflow** (`python -m crackerjack --run-tests`)
