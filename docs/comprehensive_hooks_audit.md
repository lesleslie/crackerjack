...
2. **`issues_found`** (list of strings):
...
2. **🚨 Refurb timeout too high (660s → 240s recommended)**
...
2. **Reduce refurb timeout from 660s to 240s** in `hooks.py:280`
...
4. Document why Zuban uses 40s in hooks.py vs 180s in default config (Rust performance)

### Documentation

5. Add comment in `hooks.py` explaining timeout rationale for each hook
6. Document metric semantics in `CLAUDE.md` or architecture docs
