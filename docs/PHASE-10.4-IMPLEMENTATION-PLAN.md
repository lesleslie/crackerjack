...
3. **Missing Phase 10.3 Integration**
...
2. Calculate 95th percentile execution times
3. Set timeouts to 3x P95 (safety margin for slower systems)
4. Update `hooks.py` with calibrated values
5. Add comments documenting the profiling data source
...
2. Fix skylos command mismatch
3. Standardize argument patterns (all tools use same style)
4. Ensure exclude patterns are consistent
5. Update tests to validate command consistency
...
2. Update `WorkflowOrchestrator` to use `EnhancedHookExecutor`
3. Wire `--tool` CLI flag to `ToolFilter.tool_name`
4. Wire `--changed-only` CLI flag to `ToolFilter.changed_only`
5. Implement file path passing to tools that support it
6. Add performance reporting at end of hook execution
...
2. **Project-level tools** (must run on entire project): zuban, skylos, creosote
3. **Formatting tools** (modify files in place): ruff-format, trailing-whitespace
...
2. Mark file-level tools: ruff-check, ruff-format, bandit, codespell, mdformat
3. Implement `build_command()` method
4. Update `EnhancedHookExecutor` to pass filtered files
5. Add tests for file path handling
...
**Implementation**:
```python
def optimize_hook_order(hooks: list[HookDefinition], profiler: ToolProfiler) -> list[HookDefinition]:
...
2. Update FAST_STRATEGY to use optimized ordering
3. Implement parallel execution for independent tools
4. Add `--parallel` / `--serial` CLI flags for user control
...
2. **Performance**: 70%+ improvement in execution time with optimizations enabled
3. **Usability**: `--tool` and `--changed-only` work seamlessly
4. **Maintainability**: Single source of truth for hook commands
5. **Visibility**: Performance metrics displayed after execution
...
2. **Performance Regressions**: Comprehensive benchmarking before/after
3. **Cache Correctness**: Extensive testing of cache invalidation logic
4. **Parallel Execution Bugs**: Start with serial by default, opt-in to parallel
