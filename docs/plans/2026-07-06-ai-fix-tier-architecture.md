...
2. **Adding tier 3** — spawn full Claude/Qwen CLI sessions with the actual
   Read/Edit/Bash toolset for the long tail of narrowing/typing work.
3. **Capturing every tier-3 success as a Session-Buddy skill** so the
...
2. **Coverage with tier 3:** classifier reports tier 1+2+3 ≥ 95%.
3. **Skill capture:** every tier-3 success appears in
   `Session-Buddy.search_distilled_skills` within 24h.
4. **Steady-state cost:** second-run cost (with skills replayed) is
   ≤20% of first-run cost.
5. **No regressions:** existing `crackerjack run` continues to pass
   the full test suite.
