...
2. **QualityIntelligenceService.generate_ml_recommendations** - Complexity 22
...
4. **TestManager._extract_structured_failures** - Complexity 21
...
5. **FileSystemService.read_lines_streaming** - Complexity 19
...
6. **FileSystemService.read_file_chunked** - Complexity 19
...
8. **FileSystemService.read_file** - Complexity 17
   - File: `crackerjack/services/filesystem.py:21`
   - Current: 17 → Target: ≤15
   - Strategy: Extract validation and error handling
   - Reuse helpers from read_lines_streaming

9. **FileSystemService._perform_file_copy** - Complexity 17
...
10. **EnhancedFileSystemService._read_file_direct** - Complexity 17
...
2. Verify complexity with: `python -m crackerjack`
3. Ensure no behavior changes with integration tests
4. Check coverage ratchet is maintained

## Implementation Order
1. Start with highest complexity (AsyncTimeoutManager.timeout_context)
2. Work through FileSystemService methods (5 functions) as a group
3. Complete remaining high-complexity functions
4. Finish with lowest complexity violation
