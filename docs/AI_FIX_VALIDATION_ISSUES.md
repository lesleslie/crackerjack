...
2. Orphaned return statements (wrong indentation levels)
3. Duplicate code blocks
4. Broken function definitions
5. Unclosed try/except blocks
6. Missing function parameters
...
2. **`agents/dependency_agent.py`** (5 error locations)
...
8. **`services/ai/embeddings.py`** (4 error locations)
...
9. **`services/testing/test_result_parser.py`** (7 error locations)
...
10. **`shell/adapter.py`** (1 error location)
...
3. **Run with increased limit**:
   ```bash
   python -m crackerjack run --ai-fix --comp --ai-max-iterations 5
   ```

4. **After AI-fix completes**, validate syntax:
   ```bash
   python -m compileall crackerjack -q
   ```

5. **If syntax errors occur**, revert immediately:
   ```bash
   git stash pop
   ```
...
2. ❌ **TODO**: Add syntax validation after AI agent fixes
3. ❌ **TODO**: Increase AI-fix iteration limit to 5-10
4. ❌ **TODO**: Add pre-apply validation in AgentContext
5. ❌ **TODO**: Implement automatic rollback mechanism
...
2. **Increase convergence limit** (easy win)
3. **Improve agent prompts** (longer term)
4. **Test with validation** before full deployment
