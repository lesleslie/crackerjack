...
2. `services/filesystem.py` - File operations (CRITICAL)
3. `services/cache.py` - Caching system (CRITICAL)
4. `services/config_merge.py` - Configuration (HIGH)
5. `services/logging.py` - Logging (HIGH)
6. `services/coverage_ratchet.py` - Quality enforcement (HIGH)
...
2. Convert configuration tests to synchronous
3. Keep async only where truly needed (I/O operations, timeouts)
...
2. `enhanced_coordinator.py` - Enhanced coordination (CRITICAL)
3. `security_agent.py` - Security fixes (HIGH)
4. `refactoring_agent.py` - Complexity reduction (HIGH)
5. `performance_agent.py` - Performance optimization (HIGH)
6. `dry_agent.py` - Code duplication (MEDIUM)
7. `formatting_agent.py` - Style violations (MEDIUM)
8. `import_optimization_agent.py` - Import cleanup (MEDIUM)
9. `test_creation_agent.py` - Test generation (MEDIUM)
10. `test_specialist_agent.py` - Advanced testing (MEDIUM)
11. `documentation_agent.py` - Documentation (LOW)
12. `semantic_agent.py` - Semantic analysis (LOW)
13. `architect_agent.py` - Architecture patterns (LOW)
14. `enhanced_proactive_agent.py` - Proactive prevention (LOW)
...
2. Delete if obsolete/redundant
3. Document why skipped if legitimately untestable
...
2. **Test `services/security.py`** (3 hours)
   - Critical security component
   - High value, relatively straightforward

3. **Test `services/git.py`** (4 hours)
   - Core functionality
   - Many integration tests already exist (can extract patterns)

4. **Remove Duplicates** (15 minutes)
   - Clean up `(1).py` files

5. **Create Test Organization Structure** (30 minutes)
...
3. ✅ Be independent (no test order dependencies)
4. ✅ Use proper fixtures (protocol-based DI)
5. ✅ Have clear docstrings explaining what is tested
6. ✅ Use appropriate markers (@pytest.mark.unit, etc.)
7. ✅ Follow naming: `test_<action>_<scenario>`

**Avoid**:
1. ❌ Async tests that can hang
2. ❌ Over-mocking (test behavior, not implementation)
3. ❌ Tautological assertions
4. ❌ Testing DI implementation details
5. ❌ Blanket pytest.skip() for entire classes
6. ❌ Builtin injection
7. ❌ Complex fixture dependency chains
...
2. Test critical security components (0% → 80%+)
3. Remove duplicate files
4. Test core services (git, filesystem, cache)
...
2. Create tracking board for phases
3. Begin Phase 1 implementation
4. Set up daily coverage monitoring
